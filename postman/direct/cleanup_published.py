#!/usr/bin/env python3
"""Remove one retained Direct Postman result worktree only after its PR is merged."""
...
def cleanup(
    *,
    ...
    for key in ("repository", "prNumber", "commitSha", "branch", "worktree"):
    ...
    repo_root = Path(published.get("repoRoot") or published_json.parents[3]).resolve()
    ...
    result = common.json_result(
        ...
        workspaceRegistrationRemovalRequired=True,
    )
    common.atomic_write_json(published_json.parent / "cleanup.json", result)
    return result
