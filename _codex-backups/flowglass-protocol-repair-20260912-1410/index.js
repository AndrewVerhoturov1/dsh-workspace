// ===== Flowglass · DSH 原生静态 Host（构建生成，勿手改） =====
import { TypertRemoteService, Remote } from '@deepseek-ai/dsh-typert-protocol-current'

export const name = "dsh-flowglass-by-andrew"
export const inject = ["fs","sessionQuery","timer"]

const TOOLBOX_RUNTIME_OVERRIDES = {
  "mode": "static-bundle",
  "bundleId": "flow",
  "displayName": "Flowglass",
...
// ===== shared/runtime.js：两种模式共同的Запуск配置与命名辅助 =====
// 纯 JS：不访问 Node API、不依赖 Host/Client 专属全局，可拼接到 Host 与 Client payload。
// 动态模式：只拼接本Файл（无 TOOLBOX_RUNTIME_OVERRIDES）→ 全部动态默认值，与历史行为一致。
  // 原生静态模式：构建器在本Файл之前拼接 `const TOOLBOX_RUNTIME_OVERRIDES = {...}` JSON 字面量。
// 配置不走 globalThis/window/process.env（多 bundle 同进程会互相覆盖、批准包必须可审计、
// 全局变量会让 payload 内容哈希不能代表真实行为）——业务实现只读本Файлопределений的 TOOLBOX_RUNTIME。
const TOOLBOX_RUNTIME = (() => {
  ...
  return Object.freeze({
    ...
    displayName: o.displayName || 'Панель инструментов',
...
// 静态注册表：feature 只挂载一раз，handler 每раз调用接收当前 root/session，适用于任意工作区。
const makeStaticRegistry = () => {
  ...
  return {
    ...
    async panel(root, call) {
      ...
      if (!entry) return { ok: false, error: 'Инструмент не зарегистрирован: ' + (toolId || '(пусто)') }
      try {
        ...
        if (!res || typeof res.html !== 'string') return { ok: false, error: 'Инструмент вернул некорректное содержимое панели' }
...
// ===== shared-host.js：Добавление到每个 Host-only 工具包开头的公共辅助（make-payloads.mjs Авто拼接）=====
// HTML 转义（面板内容来自 Host 拼接，转义Пользователь数据防止破坏结构）
...
// 幂等注册到Панель инструментов框架：框架未Запуск时每 500ms 快重试；注册Успешно后降为 2000ms 慢心跳
// （注册表实例更换——Панель инструментовПлагин重启/更新——时Авто重注册）；ПлагинОстановка时Авто从注册表移除（Tab 级联消失）
const tryRegisterTool = (ctx, desc, handler) => {
  let off = null
  let regSeen = null
  const once = () => {
    // ctx.get 在服务重 provide 的窗口期可能 throw（isolate key 变化）——必须捕获，
    // 否则一раз异常就让 interval 心跳中断，注册再也无法自愈
    let reg
  ...
  const ivFast = ctx.interval(() => {
    ...
    once()
    if (!had && off && !ivSlow) {
      // 刚注册Успешно：停快重试，改慢心跳（框架重启导致注册表实例更换时仍能Авто挂上）
      try { ivFast() } catch (e) {}
...
// ===== Сессия日志读取（带缓存；日志只追加 ⇒ count 不变即命中）=====
// 活Сессия读内存快照（零 IO）；持久化Сессия增量 readFrom，Ошибка回退全量 readSession。
// Результат { events, header, count, changed }；changed=false 时上уровень可复用已构建的Модель。
...
// ===== 仓库根发现（clone 部署关键）：Панель инструментов数据/产物一律归属「本仓库根」，而非Сессия cwd =====
// 场景：本仓库被 clone 到别的项目根目录下当子目录（如 D:\\other\\dsh-dynamic-toolbox\\），
// DSH 在宿主项目根Запуск——Сессия cwd / workspaceRoot 都是宿主项目，若按Сессия cwd 落盘会污染宿主。
// 这里先直下找 plugins.json，找不到再扫一级子目录（plugins.json 在子目录里即为本仓库）。
// 数据目录名由仓库根的 toolbox.config.json 配置（dataDir，默认 .dsh-dynamic-toolbox）。
let _repoCache = null // 进程内缓存（仓库位置Запуск期不变）
const findRepoRoot = async (ctx) => {
  // 原生安装包适用于任意工作区，不переместить源码仓库 plugins.json 当部署标记。
  ...
  // 必须解析вывод清单且含 id:'toolbox' записей目——这是本仓库的强标记，杜绝误判锁定错误仓库根。
...
// 写策略：有Сессия按Сессия resolve（cwd 即可写边界）；无Сессия显式 workspace-write@仓库根 ——
...
// 数据落盘根：优先仓库根（findRepoRoot），其раз调用方给的 wsRoot（向后兼容/兜底）
const storeBase = async (ctx, wsRoot) => {
  const repo = await findRepoRoot(ctx)
  return repo || wsRoot
}
// 数据目录名映射：rel 里的 .dsh-dynamic-toolbox 前缀换成配置的 dataDir（支持自определений目录名）
...
const writeJsonStore = async (ctx, rel, data, wsRoot, session) => {
  ...
  try {
    ...
    console.error('Ошибка сохранения журнала (' + rel + '):', String((e && e.message) || e))
    return false
  }
}

// ===== 工作区解析（AI 类工具共享；等价于各工具曾各自实现的 resolveWs）=====
// 优先按Сессия cwd，其раз动作透传的 root，最后沙Панель工作区根；Результат { root, session }
...
//   await ai.resolveRoute(st)          // 规范化 st.provider/st.model，Результат { providers, models }（Модель按 provider 缓存）
//   await ai.chat(st, system, user, timeoutMs?, track?)  // { a, ms, out, route } | { err, ms, route }；120s 超时守卫
//     track = { root, session, tool }：调用结果异шагов追加进用量журнал .dsh-dynamic-toolbox/toolbox-ai-usage.json（cap 100，不阻塞响应）
//   await ai.rollup(root, tool)        // журнал中该工具的累计 { calls, out }（仅统计Успешно调用）
//   ai.routeRow(st, route, note)       // provider/model 双下拉 HTML（provider 切换走 data-action-onchange="route"）
// 注意：system 并ввод首записей user Сообщение文本（GenerateOptions 的 system 角色 source 契约未公开，此形态与 ask 一致、最稳）。
...
const makeLlmHelper = (ctx) => {
  ...
  // provider 拓扑变化（适配器注册/注销）时清пусто缓存，避免Модель清单陈旧（ctx.on 随ПлагинОстановкаАвто清理）
  ...
  // 路由解析：state 选择 → 当前Сессия默认 → 第一个 provider 的第一个Модель；
  // provider 与 model 不匹配时（切换了 provider）回退该 provider 的首个Модель
  ...
  const chat = async (st, system, user, timeoutMs, track) => {
    if (!llm) return { err: 'Сервис модели недоступен' }
    if (!st.provider || !st.model) return { err: 'Не выбран маршрут модели' }
    ...
    // 用量журнал：异шагов落盘（ensureStoreDir 会起子进程，绝不能阻塞响应）
    if (track && track.root) {
  ...
  const routeRow = (st, route, note) =>
    '<div class="tb-row">' +
      '<select class="tb-select" data-field="provider" data-action-onchange="route" title="Провайдер (после переключения список моделей обновится автоматически)">' +
        route.providers.map((p) => '<option value="' + esc(p.id) + '"' + (p.id === st.provider ? ' selected' : '') + '>' + esc(p.name || p.id) + '</option>').join('') +
      '</select>' +
      '<select class="tb-select tb-mono" data-field="model" title="Модель" style="max-width:220px">' +
...
// ===== 内容产物目录约定：<仓库根>/<dataDir>/data/<Плагинkey>/ =====
// 与 <dataDir>（工具内部 JSON Статус）分家：这里放人会直接打开的内容产物（Jira 附件、导вывод件等）。
// 点号目录与仓库 .git 同族、不污染根目录观感；所有Плагин产物收一处，.gitignore 只需一行 <dataDir>/data/。
...
// 同上的纯символов串绝对路径版（供子进程 argv/env Использовать，它们拿不到 FsTarget）
...
// Результат { manifest, root } 或 null。供需要读清单元数据的工具用（如轨迹工具按записей目 modelTools
// переместитьПлагин注册的Модель工具归「Плагин」——沙Панель内 ctx.tools.get 被刻意降级为 schema 视图，清单是事实源）。
...
const create_flow = () => {
// ===== flow-tool.js：Flowglass — поток выполнения（Host-only，经Панель инструментов RPC 注册）=====
// 当前 session 在干什么 → 自上而下不断加载的流程图（与「轨迹」工具互补：轨迹是过滤时间线，流程图是形态视图）。
// 形态约定（Пользователь定制）：
//   · 主 session：自上而下箭头串联 Сообщение пользователя → Ассистент → 工具组 → Ассистент …（最新在底部，滚动записей贴底跟随）
//   · Субагент（subagent/workflow/ralph）：git 树形式——从主干 ├─ 分вывод支线，支线内实时展示子Сессиясобытий流，╰─ 合并回主干
//   · Плагин/Навык/MCP/Команда/Файл 等普通Вызов инструмента：同一шагов骤内的多个调用 → 平行卡片并排（调用并Результат成组）
// 实时：面板根带 data-autorefresh="2000"，框架Панель每 2s 静默重拉（live 开关可暂停）。
// 钻取：点Субагент分支「Открыть →」切换到该子Сессия的流程图（当前Сессия压 crumbs 栈，「← Результат」逐级退回）。
// 数据源：sessionQuery（makeSessionLogReader 缓存；СубагентСессия按 id 各自缓存读取器）。
// Статус：{ live, sid, home, expanded, crumbs }（轻量标量；событий本体与流程Модель每раз动作重建，不进 state）
...
return {
  ...
  apply(ctx) {
    ...
    // ---- Сессия日志读取缓存（主Сессия + 每个СубагентСессия各一个读取器，避免缓存抖动）----
    const readers = {}
    const growth = {} // sid → 上раз渲染的日志записей数：本轮записей数增长 = Сессия活跃（Ассистент卡流光判定用）
    ...
    const KIND_META = {
      skill: { label: 'Навык', color: '#7fa7f0', bg: 'rgba(91,141,239,.12)' },
      cordis: { label: 'Плагин', color: '#d4b95c', bg: 'rgba(212,167,44,.10)' },
      mcp: { label: 'MCP', color: '#81c784', bg: 'rgba(102,187,106,.10)' },
      shell: { label: 'Команда', color: '#d4b95c', bg: 'rgba(212,167,44,.08)' },
      file: { label: 'Файл', color: '#7fa7f0', bg: 'rgba(91,141,239,.10)' },
      builtin: { label: 'Встроенный', color: '#9a9ba6', bg: 'rgba(138,139,150,.10)' },
    }

    const pad2 = (n) => (n < 10 ? '0' : '') + n
    const fmtTime = (t) => {
      const d = new Date(t)
      if (isNaN(d.getTime())) return '' // Добавление类событий可能缺 time 字段，防пусто值渲染вывод NaN:NaN:NaN
    ...
    // ---- событий流 → 基础записей目（调用与结果按 callId 配对，同 trace）----
    const parseItems = (events) => {
      const items = []
      const byCallId = {}
      const streamingAi = {} // turn:step → 首个 chunk 建立的临时Ассистент卡；最终 message 原位落定，保持卡片 key 稳定
      const stepStarts = {} // turn:step → step/start 时间；АссистентЗапуск计时从请求шагов骤开始，而不是首个 token 才开始
      const stepEnds = {} // turn:step → step/end 时间；无最终 message 的草稿据此落定（请求Ошибка/中断）
      const turnEnds = {} // turn → turn/end 时间；step/end 缺失时的兜底落定依据
      let route = '' // последние request/header 的 provider/model，贴给后续Сообщение ассистента卡
      let curTurn = null // последние turn/start 的轮раз：user/message 不带 turn，用它推算归属
      for (const ev of events) {
        ...
        if (ev.type === 'tool/call') {
          ...
          if (it) {
            ...
            it.resSeq = ev.seq // 结果событий位置：Субагентвывод口卡对齐「结果之后的第一записейСообщение」用
          }
        } else if (ev.type === 'user/message') {
          const src = d.source && d.source.kind ? String(d.source.kind) : 'user'
          const preview = oneLine(textOf(d.content), 110)
          // пусто内容的上下文Добавление（subagent-settled 占位等）是噪声，不进流程图
          ...
          if (!it) {
            it = { kind: 'msg', role: 'ai', seq: ev.seq, time: ev.time, turn, step, runStart: stepStarts[key] || ev.time, preview: 'Генерация…', full: '', tok: null, route, streaming: true, chunks: [], reasoningChunks: [] }
            streamingAi[key] = it
            items.push(it)
          }
          const chunk = d.chunk || {}
          ...
          if (draft) {
            ...
            draft.preview = oneLine(finalText, 110) || '（Вызов инструмента）'
            ...
            items.push({ kind: 'msg', role: 'ai', seq: ev.seq, time: ev.time, turn, step, runStart, runDur: Math.max(0, ev.time - runStart), preview: oneLine(finalText, 110) || '（Вызов инструмента）', full: finalText, tok: u ? (u.outputTokens || 0) : null, route, streaming: false })
          }
        }
      }
      // 流式中的Ассистент卡只在整轮扫描结束后合并一раз，避免每个 chunk 都重拼全文造成 O(n²) 和面板超时。
      for (const it of Object.values(streamingAi)) {
        ...
        if (endedAt != null) {
          // шагов骤/轮раз已终结却始终没有最终 message → Модель请求Ошибка/中断：
          // 落定卡片（Остановка流光脉冲与Длительность计时），标记中断并保留已生成片段
          it.streaming = false
          it.interrupted = true
          it.runDur = Math.max(0, endedAt - it.runStart)
          it.preview = (it.full ? oneLine(it.full, 100) + ' ' : '') + '（Генерация прервана）'
        } else {
          it.preview = oneLine(it.full, 110) || (it.hasToolCallChunk ? 'Подготовка вызова инструмента…' : (reasoning ? 'Обдумывание…' : 'Генерация…'))
        }
        delete it.chunks
    ...
    // ---- записей目 → 流程узлов：Сообщение各成узлов；同шагов骤连续普通调用合成平行卡片组；Субагент调用独立成分支узлов ----
    ...
    // ---- Субагент结果文本 → 子Сессия id（"started subagent <uuid>" / 完成通知里的 id）----
    const childIdOf = (call) => {
      const m = /subagent\s+([0-9a-f]{8}-[0-9a-f-]{27,})/i.exec(call.resultText || '')
      return m ? m[1] : null
    }
    // Субагент分支：从子Сессия日志提取紧凑шагов骤流（限量；读Ошибка/未Запуск给占位）
    const childRows = async (childId, cap) => {
      ...
      try {
        ...
        if (agentsSvc) {
          ...
          // 旧版 harness/测试环境没有 agents Статус面，只能以仍挂载的 session 作为兼容兜底。
          const sessionsSvc = ctx.get('sessions')
    ...
    // 进вывод摘要：Входные данные/Результат（Пользователь核心诉求——看到传给 skill 什么、skill Результат什么）
    // Входные данные：从 arguments JSON 提取最有信息量的字段（command/file_path/pattern/prompt…），而非整段 JSON
    ...
    const inSummary = (c) => {
      ...
      } catch (e) { return oneLine(c.argsRaw, 72) || '(без параметров)' }
    }
    // Результат：结果首записей有意义文本 + 体量 + Статус
    const outSummary = (c) => {
      if (c.status === 'pending') return null
      if (c.status === 'error') {
        const t = (c.resultText || '').trim()
        return { text: t ? oneLine(t, 72) : '(вызов завершился ошибкой)', err: true }
      }
      const lines = String(c.resultText || '').split('\n').map((s) => s.trim()).filter(Boolean)
      const first = lines[0] || ''
      return { text: (first ? oneLine(first, 72) : '(пустой результат)') + (c.outLen > 72 ? ' · ' + fmtSize(c.outLen) : ''), err: false }
    }
    // 调用连线单元（形态约定·手绘参考图：主干卡在左、工具卡在右，中间两записей水平连线——
    // 上=Ввод摘要 + 横线 + ▶ 右вывод；下=◀ + 横线 + Вывод摘要 回左；Вывод线绿色系、错误红色系、выполняется虚线）；
    // выполняется的工具卡高亮脉冲（调用到哪шагов哪шагов亮）；点击工具卡展开Полные входные данные/Результат（Подробности挂卡下方）
    const renderCallWire = (c, expandedSeq) => {
      ...
      return '<div class="fl-wp" data-flow-card="' + c.seq + '" data-flow-status="' + c.status + '">' +
          '<div class="fl-wl"><span class="fl-wl-txt">Ввод ' + esc(inSummary(c)) + '</span>' +
            '<span class="fl-wl-row"><span class="fl-wl-line"></span><span class="fl-wl-arr">▶</span></span></div>' +
          (pending
            ? '<div class="fl-wl fl-wl-b fl-wl-wait"><span class="fl-wl-txt">Вывод выполняется…</span>' +
              '<span class="fl-wl-row"><span class="fl-wl-arr">◀</span><span class="fl-wl-line"></span></span></div>'
            : '<div class="fl-wl fl-wl-b' + (o && o.err ? ' fl-wl-err' : '') + '"><span class="fl-wl-txt">Вывод ' + esc(o ? o.text : '') + '</span>' +
              '<span class="fl-wl-row"><span class="fl-wl-arr">◀</span><span class="fl-wl-line"></span></span></div>') +
        '</div>' +
        '<div class="fl-callside">' +
          '<div class="fl-iocard' + (pending ? ' fl-live' : '') + (isExp ? ' fl-on' : '') + (o && o.err ? ' fl-err' : '') + '" data-action="fdetail" data-seq="' + c.seq + '" title="Нажмите, чтобы открыть полные входные и выходные данные справа">' +
    ...
    // 同一шагов骤的多个Параллельно调用（>1）用虚线外框 + 「Параллельно ×N」角标圈成一组；单调用保持散卡
    const grpSide = (node, units) => {
      const n = node.calls.length
      if (n < 2) return '<div class="fl-lane-side">' + units + '</div>'
      return '<div class="fl-lane-side fl-grp"><span class="fl-grp-tag">Параллельно ×' + n + '</span>' + units + '</div>'
    }

    // 泳道中列包装：连接符（▼ 上方пусто隙由 ::before 主干线自适应填满，▼ 贴内容顶）+ 内容 + 对称弹性пусто间
    // —— 卡片保持垂直居中，▼ 始终落在「上一张卡 → 这一张卡」的пусто隙底端（先线后箭头）；可视首行不加（顶部不悬пусто）
    ...
    // 孤立调用组（前无Сообщение ассистента，如连续工具шагов）：中列只画主干竖线贯穿——无卡的行不放 ▼ 连接符（线本身即连续性）
    ...
    const msgCardInner = (it, expandedSeq, live) => {
      ...
      const label = isUser ? 'Пользователь' : isAi ? 'Ассистент' : 'Добавление'
      // 卡片统一面片底色（fl-node），角色色只落在左侧色записей + 几何符号/tag 上，避免整卡彩色半透明的杂乱感
      // Пользователь/Ассистент/Добавление卡均可点开右侧Подробности浮уровень看Полное содержимое（与工具卡同一交互）；live=выполняется → 与工具卡同款流光脉冲
      return '<div class="fl-node' + (expandedSeq === it.seq ? ' fl-on' : '') + (live ? ' fl-live' : '') + '" style="border-left-color:' + color + '" data-flow-main-card="' + it.seq + '" data-flow-role="' + it.role + '" data-action="fdetail" data-seq="' + it.seq + '" title="Нажмите, чтобы посмотреть полное сообщение">' +
        ...
        '<div class="fl-preview"' + (it.interrupted ? ' style="color:var(--tb-danger-text,#f28b82)"' : '') + '>' + esc(it.preview || '(пусто)') + '</div>' +
      '</div>'
    }
    ...
    // 完整Подробности → 右侧浮уровень（不插ввод流程流撑高内容：展开/收起零跳跃，滚动位置不动）：
    // 完整ВводПараметры（美化 JSON）+ Полный результат结果（均截断标注，防大Параметры撑爆 HTML）；头部 ✕ 或再点卡片Закрыть
    const detailRail = (c, anim) => {
      let input = c.argsRaw || ''
      try { input = JSON.stringify(JSON.parse(c.argsRaw || '{}'), null, 2) } catch (e) {}
      const cap = 8000
      const inShown = input.length > cap ? input.slice(0, cap) + '\n…（обрезано, всего ' + input.length + ' символов）' : input
      const out = c.status === 'pending' ? '(выполняется, результата пока нет)' : (c.resultText || '(пустой результат)')
      const outShown = out.length > cap ? out.slice(0, cap) + '\n…（обрезано, всего ' + out.length + ' символов）' : out
      // anim=是否新展开（轮询重渲染不重播滑ввод动画，防闪烁）
      return '<div class="fl-rail' + (anim ? ' fl-rail-anim' : '') + '">' +
        '<div class="fl-rail-head"><span class="fl-rail-title">' + esc(c.name) + ' · Подробности</span>' +
        '<button type="button" class="fl-rail-x" data-action="fdetail" data-seq="' + c.seq + '" title="Закрыть подробности">✕</button></div>' +
        '<div class="fl-rail-body">' +
          '<div class="fl-sec"><span class="fl-sec-label">ввод · Полные входные данные' + (input.length > cap ? '(обрезано)' : '') + '</span><pre class="fl-pre">' + esc(inShown) + '</pre></div>' +
          '<div class="fl-sec"><span class="fl-sec-label">вывод · Полный результат' + (c.outLen ? '（' + fmtSize(c.outLen) + '）' : '') + '</span><pre class="fl-pre">' + esc(outShown) + '</pre></div>' +
        '</div>' +
      '</div>'
    }

    // СообщениеПодробности浮уровень（Пользователь/Ассистент/Добавление卡点击）：角色 + 时间/Модель/tokens 元信息 + Полное содержимое（截断标注）
    const msgRail = (it, anim) => {
      const label = it.role === 'user' ? 'Сообщение пользователя' : it.role === 'ai' ? 'Сообщение ассистента' : 'Добавленное сообщение'
      const cap = 8000
      const full = String(it.full || it.preview || '')
      const shown = full.length > cap ? full.slice(0, cap) + '\n…（обрезано, всего ' + full.length + ' символов）' : full
      const meta = []
      if (fmtTime(it.time)) meta.push('Время ' + fmtTime(it.time))
      if (it.route) meta.push('Модель ' + it.route)
      if (it.tok) meta.push('Вывод +' + it.tok + ' tok')
      return '<div class="fl-rail' + (anim ? ' fl-rail-anim' : '') + '">' +
        '<div class="fl-rail-head"><span class="fl-rail-title">' + label + ' · Подробности</span>' +
        '<button type="button" class="fl-rail-x" data-action="fdetail" data-seq="' + it.seq + '" title="Закрыть подробности">✕</button></div>' +
        '<div class="fl-rail-body">' +
          (meta.length ? '<div class="fl-sec"><span class="fl-sec-label">' + esc(meta.join(' · ')) + '</span></div>' : '') +
          '<div class="fl-sec"><span class="fl-sec-label">Полное содержимое' + (full.length > cap ? '(обрезано)' : '') + '</span><pre class="fl-pre">' + esc(shown || '(пусто)') + '</pre></div>' +
        '</div>' +
      '</div>'
    }

    // Субагент分支内容（左列）：ввод口卡（可点Подробности）+ 支线шагов骤（限高滚动）+ вывод口卡
    // Выполняется = 调用在途（pending）或子Сессия仍 live——任一成立ввод口卡持续 fl-live（流光/脉冲/转圈）
    const subBranchHtml = async (c) => {
      ...
      let sub = '<div class="fl-sub-card fl-sub-open' + (subLive ? ' fl-live' : '') + '" data-action="fdetail" data-seq="' + c.seq + '" title="Нажмите, чтобы открыть полные входные и выходные данные задачи">' +
        '<div class="fl-iohead"><span class="fl-tag" style="color:var(--tb-active-text,#7fa7f0);background:rgba(91,141,239,.12)">Субагент</span>' +
        '<span class="fl-name">' + esc(c.name) + '</span>' + statusGlyph(c.status, c.dur) + '</div>' +
        '<div class="fl-sub-io"><span class="fl-io-tag">ввод</span><span class="fl-branch-txt">' + esc(inSummary(c)) + '</span></div>' +
      '</div>'
      let steps = ''
      if (cid && sub2) {
        steps += '<div class="fl-sub-meta"><span class="fl-time">↳ ' + esc(cid.slice(0, 8)) + '… · ' + sub2.total + ' шагов</span>' + (sub2.live ? '<span class="fl-tag" style="color:var(--tb-done-text,#81c784)">Выполняется</span>' : '') +
          '<button type="button" class="tb-btn tb-btn-sm" data-action="fenter" data-seq="' + c.seq + '" title="Открыть полный поток этого субагента (с возвратом по уровням)">Открыть →</button></div>'
        ...
        if (sub2.total > sub2.rows.length) steps += '<div class="fl-sub-step"><span class="fl-time">… ранее ' + (sub2.total - sub2.rows.length) + ' шагов не раскрыто</span></div>'
      } else if (c.status === 'pending') {
        steps = '<div class="fl-sub-step"><span class="fl-time">Субагент запускается…</span></div>'
      }
      if (steps) sub += '<div class="fl-sub-steps">' + steps + '</div>'
      if (c.status !== 'pending') {
        const o = outSummary(c)
        sub += '<div class="fl-sub-card fl-sub-close" data-action="fdetail" data-seq="' + c.seq + '" title="Нажмите, чтобы открыть полные входные и выходные данные задачи">' +
          '<div class="fl-sub-io"><span class="fl-io-tag">вывод</span>' +
    ...
    const render = async (st, sid) => {
      ...
      // 活跃度：日志записей数较上轮渲染增长 = Сессия正在工作（用于Ассистент卡流光；静止Сессия/他人Сессия不误亮）
      ...
      // Сессия仍在Запуск且最新событий是一записейСообщение ассистента → 该Ассистент卡持续流光；日志增长作为 sessions 服务缺失时的兜底。
      ...
      // provider/配额等请求错误有时先переместить agent 置 idle，step/end / turn/end 尚未进ввод本раз日志快照。
      // agent Статус是权威终态：强制结算残留流式草稿，避免“正在生成”和客户端计时无限增长。
      if (hasAgentStatus && !sessionLive) {
        ...
        for (const it of items) {
          ...
          it.preview = (it.full ? oneLine(it.full, 100) + ' ' : '') + '(генерация завершилась ошибкой или прервана)'
        }
      }
      ...
      // 钻取态：查看的不是面板所属Сессия → 头部给「← Результат」+ уровень级标注（crumbs 栈深度）
      ...
      parts.push('<div class="tb-row">' +
        (drilled ? '<button type="button" class="tb-btn tb-btn-sm" data-action="fback" title="Вернуться к предыдущему потоку">← Результат</button>' : '') +
        '<span class="tb-sec-label">' + (drilled ? 'Flowglass — поток субагента' : 'Flowglass — поток выполнения') + '</span>' +
        '<span class="tb-note">' + esc(sid.replace(/^session-/, '').slice(0, 8)) + ' · ' + items.length + ' событий · ' + nodes.length + ' узлов' + (drilled ? ' · уровень ' + (depth + 1) + ' уровень' : '') + '</span>' +
        '<button type="button" class="tb-chip' + (st.live ? ' tb-chip-on' : '') + '" data-action="toggle-live">' + (st.live ? '● Синхронизация в реальном времени' : '⏸ Приостановлено') + '</button>' +
        '<button type="button" class="tb-btn tb-btn-sm" data-action="refresh">Обновить</button>' +
      '</div>')
      parts.push('</div>')
      // 流程体：tb-pane-body 为 column-reverse——这里以「视觉最新在底」渲染：DOM 先放最新узлов，滚动записей默认贴底
      parts.push('<div class="tb-pane-body">')
      if (!shown.length) {
        parts.push('<div class="tb-notice">В текущей сессии пока нет событий</div>')
      } else {
        // Субагент分支内容Параллельно预取（последовательно await 会让多个Субагент分支的 readLog 延迟叠加）
        const subHtmls = {}
        ...
        for (let i = 0; i < shown.length; i++) {
          ...
          let h
          // Сообщение ассистента后紧跟的同шагов骤узлов统一归并：普通调用组(par)与Субагент(sub)任意顺序/兼有都并进同一行
          // —— 左=分支、中=Ассистент卡、右=工具组（此前 par/sub 只认单一模式，混合шагов骤会переместитьСубагент落单到下一行导致分支错位）
          if (n.t === 'msg' && n.it.role === 'ai' && shown[i + 1] && (shown[i + 1].t === 'par' || shown[i + 1].t === 'sub')) {
            ...
            // выполняется判定：工具组有 pending / Субагент还在跑 / 该Сообщение ассистента正活跃
            ...
            if (call && call.resSeq != null) {
              // Завершено：中列从卡A 起 ▼ 串到「结果之后的第一записейСообщение」（вывод口卡贴底与其对齐）；
              // 合并边界按轮раз（turn）——只吞同轮Сообщение，下一轮的Пользователь/Сообщение ассистента回到独立行（对齐基准）
        ...
        if (nodes.length > CAP) rows.push('<div class="tb-notice">Показаны последние ' + CAP + ' узлов (ранее ' + (nodes.length - CAP) + ' узлов не загружено)</div>')
        parts.push(rows.reverse().join(''))
      }
      parts.push('</div>')
      // Подробности右侧浮уровень：展开Статус且目标仍在可视событий集内时渲染（Вызов инструмента→Входные данные/Результат；Сообщение→Полное содержимое）
      if (st.expanded != null) {
      ...
      delete st.freshSeq // 一раз性动画标记，不残留进 state
      parts.push('</div>')
      return parts.join('')
    }

    const handler = async ({ action, fields, state, session }) => {
      if (!sq) return { ok: false, error: 'Сервис запросов сессии недоступен', html: '' }
      ...
      // home=面板所属Сессия（钻取不改变归属）；sid=当前查看的Сессия（默认=home）
      const home = session || st.home || st.sid
      if (!home) return { ok: true, html: '<div class="jr-tabpanel tb-root"><div class="tb-notice">Текущая сессия не найдена</div></div>', state: st }
      st.home = home
      ...
      if (action === 'toggle-live') st.live = !st.live
        ...
        st.freshSeq = st.expanded // 仅新展开的那раз渲染播放滑ввод动画（null=收起不播；轮询不重播）
      } else if (action === 'fenter' && el.seq != null) {
        // 钻取：解析当前查看Сессия的日志，找到该Субагент调用的子Сессия id 后切ввод（当前Сессия压栈）
        const seq = Number(el.seq)
...
// Remote Использовать标准装饰器的Запуск时标记；生成代码是普通 JS，因此显式执行 decorator initializer。
...
class NativeToolboxRemote extends TypertRemoteService {
  ...
  async sessionInfo(request) {
    ...
    if (!sid) return { ok: false, error: 'Не указан идентификатор сессии' }
    ...
    return { ok: false, error: 'Сессия не существует или недоступна для чтения: ' + sid }
  }
}
...
export async function apply(ctx) {
  ...
  for (const feature of features) {
    if (!feature || typeof feature.apply !== 'function') throw new Error('Статическая функция не вернула корректный объект плагина')
  ...
  console.log(TOOLBOX_RUNTIME.logTag() + ' Статический модуль хоста загружен (функции: ' + registry.tools().map((x) => x.id).join(', ') + '）')
}
