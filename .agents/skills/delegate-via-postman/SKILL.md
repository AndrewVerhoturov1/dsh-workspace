...
Для coding-задачи, явно отправленной через Postman, до `postman_async_send` не выполняй implementation work локально:

- не загружай frontend/design/CAD/implementation skills ради решения самой задачи;
- не исследуй структуру приложения, чтобы самостоятельно выбрать архитектуру;
- не добавляй «полезные» функции, проверки, технологии или требования, которых пользователь не задавал;
- не превращай короткое пользовательское намерение в расширенное техническое задание.

Нормализация `task` минимальна: удалить можно только управляющую формулировку, выбирающую Postman как transport, и только если смысл остаётся неизменным. Остальную формулировку и явные ограничения пользователя сохраняй максимально близко к исходному тексту.
...
Нельзя самостоятельно дописывать к этому `адаптивность`, `доступность`, конкретный набор кнопок, обработку деления на ноль, framework, архитектуру, язык ответа или формат реализации, если пользователь этого не подтверждал.
...
После trusted Runtime delivery прочитай JSON handle из результата:

- `state = RESULT_DURABLE` — использовать только указанный `resultZip`/`resultPath`, проверить пакет и внедрить его как Local Implementation Agent; не искать ZIP вручную в браузере;
- `state = WEB_WORKER_FAILED` — остановить задачу и доложить пользователю exact `request_id` и `error`; не создавать новый REQ и не обходить Postman прямым браузером.

При `RESULT_DURABLE` Л1 остаётся только интегратором:

1. проверить base/manifest/patch на совместимость с текущим repository;
2. применить подготовленный Ч1 package без самостоятельного перепроектирования;
3. запустить относящиеся к результату тесты;
4. при конфликте остановиться и доложить, а не чинить решение Ч1 «по памяти».
