...
Номер Issue и репозиторий сохраняются в `requests`. Затем тот же существующий мост [chatgpt_chat.ps1](C:/Users/andre/.dsh/chatgpt-desktop-uia-bridge/chatgpt_chat.ps1) запускается в режиме `SubmitOnly`: проверяются ChatGPT, свежий чат, точный ввод, кнопка Send и подтверждение пользовательского сообщения. Результат ChatGPT не читается и не считается ответом; нужен только подтверждённый запуск. После этого REQ становится `WAITING`, а Postman освобождается.
