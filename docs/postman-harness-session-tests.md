...
2. Bundle оставляет `agent-loop` без конфигурируемых агентов; плагин запускает POSTMAN после инъекции `sessionPersistence` через публичные `ctx.agents.resume()`/`ctx.agents.create()`.
...
Текущий набор детерминированных тестов содержит 14 проверок; отдельно проверяются заполненная pending-таблица, scoped `allow: []` и очистка корреляции после уничтожения отправителя.
