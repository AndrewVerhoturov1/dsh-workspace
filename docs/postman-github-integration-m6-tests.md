...
|---|---|---|
| Один агент → Issue WAITING → Web ChatGPT SubmitOnly | автоматическая отправка | BLOCKED: после исправления не запускался — процесс `ChatGPT (Beta)` отсутствует |
...
Live gate остаётся заблокированным внешним состоянием Desktop ChatGPT: после локального исправления проверка чтением показала, что Harness на `127.0.0.1:3080` отвечает, но процесс `ChatGPT (Beta)` отсутствует. Поэтому выполнено 0/5 новых SubmitOnly-проб, Issue/Actions/READY-контур после исправления не запускался, а прежний отказ Issue №19 (`FRESH_CHAT_NOT_CONFIRMED: EMPTY_SURFACE_MARKER_NOT_FOUND`) не засчитывается как проверка исправленного кода.

## Итог

`BLOCKED — процесс ChatGPT (Beta) отсутствует, поэтому пять живых SubmitOnly-проб и полный M6 после исправления не доказаны; локальные проверки проходят.`
