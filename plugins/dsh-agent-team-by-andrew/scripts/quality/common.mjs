...
export async function resolveRepositoryReleaseTarball(value) {
  ...
  invariant(dirname(candidate) === dist, 'release tarball must be a direct file below the repository dist directory')
  invariant(basename(candidate) === `${manifest.name.replace(/^@/, '').replaceAll('/', '-')}-${manifest.version}.tgz`, 'release tarball filename does not match package version')
...
export class CommandRunner {
  ...
  async run(command, args, { capture = false, timeoutMs = 300_000 } = {}) {
    // Windows cannot spawn pnpm.cmd without a shell. Reuse the exact CLI that
    // launched this quality script, rather than interpreting arguments through cmd.exe.
    if (process.platform === 'win32' && command === 'pnpm') {
      const cli = process.env.npm_execpath
      invariant(cli && isAbsolute(cli) && /^pnpm\.(?:c?js)$/.test(basename(cli)),
        'On Windows run quality scripts through pnpm run (an absolute pnpm CLI is required)')
      args = [cli, ...args]
