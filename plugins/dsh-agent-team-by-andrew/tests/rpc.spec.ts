...
describe('agent team RPC handler', () => {
  ...
  it('serves a snapshot with agents, squads, and the model catalog', async () => {
    ...
    if (!result.ok) return
    expect(result.value.apiVersion).toBe(6)
  ...
  it('round-trips persistent-lead conversation routing through RPC', async () => {
    const state = createService()
    const implementerId = 'implementer' as AgentId
    const bridgeId = 'postman-bridge' as AgentId
    await state.agents.put(implementerId, { ...agent('Implementer'), reasoningEffort: 'high' })
    await state.agents.put(bridgeId, { ...agent('Postman Bridge'), model: 'cheap', reasoningEffort: 'low', kind: 'postman-bridge' })
    const handler = createAgentTeamRpcHandler(state.ctx, state.service)
    const created = await call<{ id: string }>(handler, 'squad/create', {
      record: {
    ...
    expect(created.ok).toBe(true)
    if (!created.ok) return
    const snapshot = await call<SnapshotValue & { squads: Array<Record<string, unknown>> }>(handler, 'snapshot', {}, signal())
    expect(snapshot.ok && snapshot.value.squads.find(item => item.id === created.value.id)).toMatchObject({
      conversationMode: 'persistent-lead', leaderAgentId: implementerId,
    })
  })
