...
describe('release engineering contract', () => {
  ...
  it('enforces the active fork CI rather than relying on nested workflows', () => {
    const source = readFileSync(join(root, '..', '..', '.github', 'workflows', 'agent-team-quality.yml'), 'utf8')
    expect(source).toContain('branches: [preview, main]')
    expect(source).toContain('working-directory: plugins/dsh-agent-team-by-andrew')
    expect(source).toContain('contents: read')
    expect(source).toContain('persist-credentials: false')
    expect(source).toContain('fetch-depth: 0')
    expect(source).toContain('os: [ubuntu-latest, windows-latest]')
    expect(source).toContain("node: ['22.19.0', '24']")
    expect(source).toContain('pnpm install --frozen-lockfile --ignore-scripts')
    for (const command of ['check:whitespace', 'typecheck', 'test', 'build', 'audit:pack']) {
      expect(source).toContain(`- run: pnpm run ${command}\n`)
    }
    expect(source.indexOf('- run: pnpm run build')).toBeLessThan(source.indexOf('- run: pnpm run audit:pack'))
    expect(source).not.toMatch(/continue-on-error|pull_request_target|smoke:install|smoke:browser|postman/i)
