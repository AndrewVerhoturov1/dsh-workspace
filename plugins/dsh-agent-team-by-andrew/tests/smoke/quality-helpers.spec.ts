...
describe('quality script safety helpers', () => {
  ...
  it('rejects release-artifact overrides outside the repository dist directory', async () => {
    await expect(resolveRepositoryReleaseTarball(join(tmpdir(), 'untrusted-release.tgz'))).rejects.toThrow(/repository dist directory/)
    await expect(resolveRepositoryReleaseTarball('dist/nested/release.tgz')).rejects.toThrow(/repository dist directory/)
    await expect(resolveRepositoryReleaseTarball('dist/wrong-package-0.0.0.tgz')).rejects.toThrow(/filename does not match/)
  })
  ...
  it('cleans only tool-owned temporary workspaces', async () => {
    ...
    await expect(new TemporaryWorkspace(tmpdir()).cleanup()).rejects.toThrow(/refusing/)
    const owner = await TemporaryWorkspace.create()
    try {
      const foreign = await owner.directory('not-tool-owned')
      await expect(new TemporaryWorkspace(foreign).cleanup()).rejects.toThrow(/refusing/)
  ...
  it('isolates every user/config/cache root and removes credentials', async () => {
    ...
    try {
      ...
      for (const key of [
        ...
        expect(env[key]?.startsWith(`${workspace.root}${sep}`)).toBe(true)
      }
  ...
  it('runs the invoking pnpm CLI without a command shell', async () => {
    const result = await new CommandRunner().run('pnpm', ['--version'], { capture: true })
  ...
  it('parses a successful RPC body exactly once and reads error text only on failure', async () => {
    ...
    try {
      globalThis.fetch = async () => new Response(JSON.stringify({
        ...
        result: { ok: true, value: { apiVersion: 6 } },
      }), { status: 200, headers: { 'content-type': 'application/json' } })
      const now = Date.now
      Date.now = () => 1
      try {
        await expect(fixture.rpc('snapshot')).resolves.toEqual({ apiVersion: 6 })
      } finally {
        Date.now = now
      }
