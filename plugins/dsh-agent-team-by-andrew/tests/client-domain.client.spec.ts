...
import { agentDraftOf, EMPTY_AGENT, EMPTY_SQUAD, squadDraftOf, toAgentRecord, toSquadRecord, validateAgent, validateSquad } from '../src/client/forms.ts'
...
describe('client domain validation and view models', () => {
  ...
  it('round-trips and validates dedicated persistent Implementer conversation routing', () => {
    const agents = [
      { id: 'impl', name: 'Implementer', systemPrompt: 'Apply external artifacts.', provider: 'p', model: 'expensive', reasoningEffort: 'high' },
      { id: 'bridge', name: 'Postman Bridge', systemPrompt: 'Transport only.', provider: 'p', model: 'cheap', reasoningEffort: 'low', kind: 'postman-bridge' as const },
    ]
    const draft = {
      ...EMPTY_SQUAD,
      ...
      members: ['impl', 'bridge'],
      leaderAgentId: 'impl',
      conversationMode: 'persistent-lead' as const,
      triggerMode: 'guaranteed' as const,
      activationMode: 'always' as const,
      responseMode: 'foreground' as const,
    }
    expect(validateSquad(draft, agents).valid).toBe(true)
    expect(toSquadRecord(draft)).toMatchObject({ conversationMode: 'persistent-lead', leaderAgentId: 'impl' })
    expect(squadDraftOf({ id: 'team', name: draft.name, members: draft.members, collabNote: '', conversationMode: 'persistent-lead', leaderAgentId: 'impl' }))
      .toMatchObject({ conversationMode: 'persistent-lead', leaderAgentId: 'impl' })
    expect(validateSquad({ ...draft, leaderAgentId: 'bridge' }, agents).errors.leaderAgentId).toBe('persistentLeadBridgeError')
    expect(validateSquad({ ...draft, responseMode: 'background' }, agents).errors.responseMode).toBe('persistentLeadForegroundOnly')
    expect(validateSquad({ ...draft, activationMode: 'manual' }, agents).errors.activationMode).toBe('persistentLeadAlwaysOnly')
  })

  it('round-trips member kind and primary/fallback reasoning effort without changing legacy standard records', () => {
    const draft = agentDraftOf({
      id: 'bridge',
      name: 'Postman Bridge',
    ...
    expect(draft).toMatchObject({ reasoningEffort: 'low', kind: 'postman-bridge', fallbackReasoningEffort: 'off' })
    expect(toAgentRecord(draft)).toMatchObject({ reasoningEffort: 'low', kind: 'postman-bridge', fallbackReasoningEffort: 'off' })
    const ordinary = toAgentRecord({ ...EMPTY_AGENT, name: 'Ordinary', systemPrompt: 'Role', provider: 'p', model: 'm' })
    expect(ordinary).not.toHaveProperty('kind')
    expect(ordinary).not.toHaveProperty('reasoningEffort')
  })
