...
describe('AgentTeamService CRUD', () => {
  ...
  it('validates a dedicated persistent Implementer Team and renders root-shell guidance', async () => {
    const state = createService()
    const implementerId = AgentId('implementer')
    const bridgeId = AgentId('postman-bridge')
    await state.agents.put(implementerId, { ...agent('Implementer'), reasoningEffort: 'high' })
    await state.agents.put(bridgeId, { ...agent('Postman Bridge', 'cheap-model'), reasoningEffort: 'low', kind: 'postman-bridge' })
    const teamId = SquadId('postman-team')
    await state.service.createSquad({
      name: 'Postman Implementation',
      members: [implementerId, bridgeId],
      leaderAgentId: implementerId,
      conversationMode: 'persistent-lead',
      triggerMode: 'guaranteed',
      activationMode: 'always',
      responseMode: 'foreground',
    }, teamId)
    await state.service.setSessionSquadMode(state.parent.id, teamId)
    const guidance = state.service.squadModeGuidance(state.parent)
    expect(guidance).toContain('root conversation Agent is only the user-facing shell')
    expect(guidance).toContain('It is NOT the Implementer')
    expect(guidance).toContain('Implementer')
    await expect(state.service.updateSquad(teamId, {
      name: 'Bad persistent Team', members: [implementerId, bridgeId], leaderAgentId: bridgeId, conversationMode: 'persistent-lead',
    })).rejects.toMatchObject({ code: 'INVALID_MEMBERS' })
  })

  it('validates and preserves per-member reasoning effort for primary and fallback routes', async () => {
    ...
    const id = AgentId('reasoning-member')
    await state.service.createAgent({
      ...agent('Reasoner'),
    ...
    expect(state.service.getAgent(id)).toMatchObject({ reasoningEffort: 'high', fallbackReasoningEffort: 'low' })
    await expect(state.service.updateAgent(id, { ...agent('Reasoner'), reasoningEffort: 'unsupported' }))
      .rejects.toThrow('reasoning effort "unsupported" is not supported')
  })
})


describe('persistent Implementer conversation routing', () => {
  it('keeps one continuable Implementer across turns and authorizes only its same-Team Bridge', async () => {
    const live = new Map<string, Agent>()
    let state: ReturnType<typeof createService>
    let implementerTurn = 0
    const continuableStarts: Parameters<SubagentRuntime['startContinuable']>[0][] = []
    const sendCalls: Array<{ target: string; text: string }> = []
    const startContinuable: SubagentRuntime['startContinuable'] = async (spec) => {
      continuableStarts.push(spec)
      implementerTurn += 1
      const messageId = MessageId(`impl-message-${implementerTurn}`)
      const child = {
        id: spec.childId!,
        options: spec.request.agentOptions ?? {},
        session: {
          header: { parentSession: spec.request.parent.id, origin: 'subagent', delegationDepth: 1 },
          events: [
            { type: 'user/message', data: { id: messageId } },
            { type: 'assistant/message', data: { message: { content: [{ type: 'text', text: `implementer-result-${implementerTurn}` }] } } },
          ],
        },
        async whenIdle() {},
      } as unknown as Agent
      live.set(String(child.id), child)
      return { childId: child.id, messageId }
    }
    const followup: SubagentRuntime['followup'] = async (_sender, targetId, content, options) => {
      expect(options.source).toMatchObject({ kind: 'coordinator', form: 'relay', senderSessionId: state.parent.id })
      const child = live.get(String(targetId))
      if (child === undefined) throw Object.assign(new Error('not resumable'), { code: 'NOT_RESUMABLE' })
      implementerTurn += 1
      const messageId = MessageId(`impl-message-${implementerTurn}`)
      sendCalls.push({ target: String(targetId), text: content.filter(block => block.type === 'text').map(block => block.text).join('') })
      const events = (child.session as unknown as { events: unknown[] }).events
      events.push(
        { type: 'user/message', data: { id: messageId } },
        { type: 'assistant/message', data: { message: { content: [{ type: 'text', text: `implementer-result-${implementerTurn}` }] } } },
      )
    ...
    state = createService({
      agentsGet: sessionId => live.get(String(sessionId)),
      toolSchemas: () => [
        { name: 'read_file', description: 'Read' },
        { name: 'write_file', description: 'Write' },
        { name: 'dispatch_postman_bridge', description: 'Bridge' },
        { name: 'dispatch_to_squad', description: 'Team' },
        { name: 'subagent', description: 'Delegate', parameters: { properties: { description: { type: 'string' }, prompt: { type: 'string' } }, required: ['description', 'prompt'] } },
      ],
      startContinuable,
      followup,
      start: async (provider, request) => {
        expect(provider).toBe('spawn')
        const internalName = request.toolFilter?.allow?.[0]
        const tool = state.registeredTools.get(internalName!)
        expect(tool).toBeDefined()
        const id = SessionId('nested-bridge-child')
        const child = {
          id,
          session: { header: { parentSession: request.parent.id, origin: 'subagent', delegationDepth: 2 } },
        } as unknown as Agent
        await tool!.execute({}, { agent: child, signal: request.signal } as never)
        state.starts.push({ provider, request })
        return {
          id,
          localAgent: child,
          result: Promise.resolve({ output: [{ type: 'text' as const, text: 'transport settled' }], stopReason: 'completed' as const }),
          async dispose() {},
        }
      },
    })
    live.set(String(state.parent.id), state.parent)
    const implementerId = AgentId('implementer')
    const bridgeId = AgentId('bridge')
    const teamId = SquadId('postman-team')
    await state.agents.put(implementerId, {
      name: 'Implementer', systemPrompt: 'Apply external artifacts only.', provider: 'configured', model: 'expensive-model', reasoningEffort: 'high',
    })
    await state.agents.put(bridgeId, {
      name: 'Postman Bridge', systemPrompt: 'Transport only.', provider: 'configured', model: 'cheap-model', reasoningEffort: 'low', kind: 'postman-bridge',
    })
    await state.squads.put(teamId, {
      name: 'Postman Implementation', members: [implementerId, bridgeId], leaderAgentId: implementerId,
      conversationMode: 'persistent-lead', triggerMode: 'guaranteed', activationMode: 'always', responseMode: 'foreground',
    })
    const route = state.service as unknown as {
      dispatchPersistentImplementerConversation(id: ReturnType<typeof SquadId>, task: string, parent: Agent, signal: AbortSignal): Promise<{ childId: SessionId; output: string }>
    }
    const signal = new AbortController().signal
    const first = await route.dispatchPersistentImplementerConversation(teamId, 'first exact task', state.parent, signal)
    const second = await route.dispatchPersistentImplementerConversation(teamId, 'second exact task', state.parent, signal)
    expect(first.childId).toBe(second.childId)
    expect(first.output).toBe('implementer-result-1')
    expect(second.output).toBe('implementer-result-2')
    expect(continuableStarts).toHaveLength(1)
    expect(sendCalls).toEqual([{ target: String(first.childId), text: 'second exact task' }])
    expect(continuableStarts[0]?.request.agentOptions).toMatchObject({ provider: 'configured', model: 'expensive-model', reasoningEffort: 'high' })
    expect(continuableStarts[0]?.request.persona).toContain('dedicated Implementer')
    expect(continuableStarts[0]?.request.persona).toContain('REPO_POLICY.md')
    expect(continuableStarts[0]?.request.persona).toContain('system/implementation-package-workflow.md')
    expect(continuableStarts[0]?.request.toolFilter?.deny).toEqual(expect.arrayContaining(['dispatch_to_squad', 'subagent']))

    const effortResolver = state.service as unknown as {
      configuredReasoningEffort(agent: Agent): string | undefined
    }
    const coldResumedImplementer = {
      id: first.childId,
      options: { provider: 'configured', model: 'expensive-model' },
      session: { header: { parentSession: state.parent.id, origin: 'subagent', delegationDepth: 1 } },
    } as unknown as Agent
    expect(effortResolver.configuredReasoningEffort(coldResumedImplementer)).toBe('high')
    const freshBridgeLikeChild = {
      id: SessionId('bridge-effort-child'),
      options: { provider: 'configured', model: 'cheap-model', reasoningEffort: 'low' },
      session: { header: { parentSession: first.childId, origin: 'subagent', delegationDepth: 2 } },
    } as unknown as Agent
    expect(effortResolver.configuredReasoningEffort(freshBridgeLikeChild)).toBe('low')

    const invoke = vi.fn(async () => ({
      ok: true, requestId: 'REQ_20260919T120000Z_0001', state: 'RESULT_DURABLE', code: 'RESULT_DURABLE', resultZip: 'D:\\results\\result.zip',
    }))
    ;(state.service as unknown as { postmanInvoker: typeof invoke }).postmanInvoker = invoke
    const implementer = live.get(String(first.childId))!
    const bridged = await state.service.dispatchPostmanBridgeFromTool({ task: 'external solution please' }, implementer, signal)
    expect(bridged).toMatchObject({ bridgeAgentId: bridgeId, bridgeAgentName: 'Postman Bridge', state: 'RESULT_DURABLE' })
    expect(state.starts[0]?.request.maxDepth).toBe(2)
    expect(state.starts[0]?.request.parent.id).toBe(first.childId)
  })
})

describe('Postman Bridge dispatch', () => {
  it('repeats serial bridge calls without the squad message claim and preserves exact task/chat correlation', async () => {
    let state: ReturnType<typeof createService>
    state = createService({
      start: async (provider, request) => {
        expect(provider).toBe('spawn')
        const internalName = request.toolFilter?.allow?.[0]
        expect(internalName).toMatch(/^postman_direct_bridge_/)
        const tool = state.registeredTools.get(internalName!)
        expect(tool).toBeDefined()
        const id = SessionId(`bridge-child-${state.starts.length + 1}`)
        const child = {
          id,
          session: { header: {
    ...
    let call = 0
    const invoke = vi.fn(async ({ task, chatRequestId }: { task: string; chatRequestId?: string }) => {
      call += 1
      return {
        ok: true,
        requestId: `REQ_20260919T01010${call}Z_000${call}`,
        state: 'RESULT_DURABLE',
        code: 'RESULT_DURABLE',
        resultZip: `D:\\results\\result-${call}.zip`,
        ...(chatRequestId === undefined ? {} : { continuedFromRequestId: chatRequestId }),
    ...
    const signal = new AbortController().signal
    const first = await state.service.dispatchPostmanBridgeFromTool({ task: 'exact first task' }, state.parent, signal)
    const second = await state.service.dispatchPostmanBridgeFromTool({
      task: 'exact follow-up task',
      chatRequestId: first.requestId,
    }, state.parent, signal)

    expect(invoke).toHaveBeenNthCalledWith(1, expect.objectContaining({ task: 'exact first task' }))
    expect(invoke).toHaveBeenNthCalledWith(2, expect.objectContaining({ task: 'exact follow-up task', chatRequestId: first.requestId }))
    expect(first).toMatchObject({ ok: true, state: 'RESULT_DURABLE', bridgeAgentId: bridgeId, bridgeAgentName: 'Postman Bridge', provider: 'configured', model: 'cheap-model', reasoningEffort: 'low' })
    expect(second).toMatchObject({ ok: true, continuedFromRequestId: first.requestId })
    expect(state.starts).toHaveLength(2)
    expect(state.starts.every(item => item.request.agentOptions?.model === 'cheap-model')).toBe(true)
    expect(state.starts.every(item => (item.request.agentOptions as { reasoningEffort?: string } | undefined)?.reasoningEffort === 'low')).toBe(true)
    expect(state.starts.every(item => item.request.maxDepth === 1)).toBe(true)
    expect(state.messageClaims.size).toBe(0)
    expect(state.registeredTools.size).toBe(0)
  })

  it('requires an explicitly marked bridge and rejects delegated callers', async () => {
    const state = createService()
    await state.agents.put(AgentId('ordinary'), agent('Ordinary'))
    await expect(state.service.dispatchPostmanBridgeFromTool({ task: 'task' }, state.parent, new AbortController().signal))
      .rejects.toMatchObject({ code: 'AGENT_NOT_FOUND' })

    await state.agents.put(AgentId('bridge'), { ...agent('Bridge'), kind: 'postman-bridge' })
    const delegated = {
      id: SessionId('delegated-parent'),
      session: { header: { origin: 'subagent', parentSession: state.parent.id, delegationDepth: 1 } },
    } as unknown as Agent
    await expect(state.service.dispatchPostmanBridgeFromTool({ task: 'task' }, delegated, new AbortController().signal))
