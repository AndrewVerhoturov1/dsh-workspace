...
export interface FixtureOptions {
  ...
  readonly startContinuable?: SubagentRuntime['startContinuable']
  readonly followup?: SubagentRuntime['followup']
  readonly interrupt?: SubagentRuntime['interrupt']
...
export interface Fixture {
  ...
  readonly registeredTools: Map<string, { readonly name: string; execute(args: unknown, exec: unknown): Promise<unknown> }>
...
export function createService(options: FixtureOptions = {}): Fixture {
  ...
  const registeredTools = new Map<string, { readonly name: string; execute(args: unknown, exec: unknown): Promise<unknown> }>()
  const notResumable = (): Error & { code: string } => Object.assign(new Error('not resumable'), { code: 'NOT_RESUMABLE' })
  const subagents = {
    ...
    startContinuable: options.startContinuable ?? (async () => { throw notResumable() }),
    followup: options.followup ?? (async () => { throw notResumable() }),
    interrupt: options.interrupt ?? (() => {}),
    getProvider: () => ({ capabilities: { agentOptions: true, outputSchema: true, depthLimit: true, toolFilter: true, persona: true } }),
  }
  const llm = {
  ...
  ctx.provide('sessions', { get: options.sessionsGet ?? (() => undefined), async flush() {} })
  ctx.provide('tools', {
    schemas: options.toolSchemas ?? (() => [{ name: 'read_file', description: 'Read a workspace file' }]),
    register(definition: { readonly name: string; execute(args: unknown, exec: unknown): Promise<unknown> }) {
      registeredTools.set(definition.name, definition)
      return () => { if (registeredTools.get(definition.name) === definition) registeredTools.delete(definition.name) }
    },
  })
  ...
  const parent = {
    ...
    session: { header: { cwd: '/workspace/project' }, requestHeader: () => undefined },
  } as unknown as Agent
  return { ctx, service, agents, squads, modes, nextModes, messageClaims, runs, versions, projectDefaults, starts, registeredTools, parent }
}
