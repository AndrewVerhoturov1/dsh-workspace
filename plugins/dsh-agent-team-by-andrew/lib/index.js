...
const DEFAULT_HANDOFF_SUMMARY_MAX_CHARS = 16e3;
/** Smallest useful configurable handoff summary. */
const MIN_HANDOFF_SUMMARY_MAX_CHARS = 1e3;
/** Hard ceiling that keeps one member from dominating stored/model-facing context. */
const MAX_HANDOFF_SUMMARY_MAX_CHARS = 32e3;
//#endregion
//#region src/spec.ts
/**
...
const squadReadFields = z$1.object({
	...
	handoffSummaryMaxChars: z$1.number().int().min(MIN_HANDOFF_SUMMARY_MAX_CHARS).max(MAX_HANDOFF_SUMMARY_MAX_CHARS).optional(),
...
function squadRecord(input) {
	return {
		...
		...input.handoffSummaryMaxChars === void 0 ? {} : { handoffSummaryMaxChars: input.handoffSummaryMaxChars },
...
function normalizeHandoff(value, fallbackText, summaryMaxChars = DEFAULT_HANDOFF_SUMMARY_MAX_CHARS) {
	if (!Number.isInteger(summaryMaxChars) || summaryMaxChars < 1e3 || summaryMaxChars > 32e3) throw new RangeError(`handoff summary limit must be ${MIN_HANDOFF_SUMMARY_MAX_CHARS}-${MAX_HANDOFF_SUMMARY_MAX_CHARS} characters`);
	...
	return {
		summary: (typeof raw["summary"] === "string" && raw["summary"].trim() !== "" ? raw["summary"].trim().slice(0, summaryMaxChars) : fallbackText.trim().slice(0, summaryMaxChars)) || "Member completed without a textual summary.",
...
var ExecutionApplicationService = class extends DefinitionApplicationService {
	...
	*/
	...
	async settleRun(member, run, attempts, startedAt, handoffSummaryMaxChars, baseline) {
		let result;
		let executionError;
		try {
		...
		const deliveredText = result === void 0 ? "" : this.resultText(result.output).trim();
		const deliveredDespiteProtocolError = result?.stopReason === "error" && deliveredText !== "";
		if (result !== void 0 && result.stopReason !== "completed" && !deliveredDespiteProtocolError) errors.push(`run ended with stop reason ${result.stopReason}`);
		...
		return {
			...
			...result === void 0 ? {} : { handoff: normalizeHandoff(result.structured, deliveredText, handoffSummaryMaxChars) }
		};
	}
	async runMember(provider, squad, member, sharedTask, chainText, parent, signal, dispatchId, attempt, route, persist = true, auxiliaryUsage) {
		...
		try {
			...
			try {
				settled = await this.settleRun(member, run, attempt, startedAt, squad.handoffSummaryMaxChars ?? 16e3, baseline);
			} finally {
