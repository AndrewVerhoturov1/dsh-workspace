...
import { createDispatchPostmanBridgeTool } from '../dispatch-postman-bridge.ts'
import { ReasoningEffortId, type ContentBlock, type UserMessage } from '@deepseek-ai/dsh-llm'
...
import { createPostmanDirectCapabilityTool, invokeDirectPostman, type DirectPostmanBridgeResult, type DirectPostmanInvoker } from '../postman-direct.ts'
...
import {
  ...
  type PostmanBridgeDispatchRequest,
  type PostmanBridgeDispatchResult,
...
interface PersistentImplementerBinding {
  readonly root: Agent
  readonly squadId: SquadId
  readonly squad: SquadRecord
  readonly leaderAgentId: AgentId
  readonly leader: AgentRecord
}

export interface PersistentImplementerConversationResult {
...
export class ExecutionApplicationService extends DefinitionApplicationService {
  ...
  /** Test seam and single authority for the host-owned canonical Direct Postman invocation. */
  protected postmanInvoker: DirectPostmanInvoker = invokeDirectPostman
  ...
  /** Stable per-root Team child identity so the dedicated Implementer survives ordinary turns and cold resume. */
  private persistentImplementerChildId(rootId: SessionId, squadId: SquadId, leaderAgentId: AgentId, leader: AgentRecord): SessionId {
    const definitionFingerprint = JSON.stringify({
      provider: leader.provider, model: leader.model, reasoningEffort: leader.reasoningEffort, maxTokens: leader.maxTokens,
      systemPrompt: leader.systemPrompt, toolScope: leader.toolScope,
    })
    const hex = createHash('sha256')
      .update(`dsh-agent-team:persistent-lead:v1:${String(rootId)}:${String(squadId)}:${String(leaderAgentId)}:${definitionFingerprint}`)
      .digest('hex')
    const uuid = `${hex.slice(0, 8)}-${hex.slice(8, 12)}-5${hex.slice(13, 16)}-a${hex.slice(17, 20)}-${hex.slice(20, 32)}`
    return uuid as SessionId
  }

  /** Resolve whether an exact delegated Agent is the configured persistent Implementer for one Team. */
  protected persistentImplementerBinding(agent: Agent): PersistentImplementerBinding | undefined {
    const parentSession = agent.session.header.parentSession
    if (parentSession === undefined || agent.session.header.origin !== 'subagent') return undefined
    const root = this.ctx.agents.get(parentSession)
    if (root === undefined) return undefined
    for (const [squadId, squad] of this.squads().entries()) {
      if (squad.conversationMode !== 'persistent-lead' || squad.leaderAgentId === undefined) continue
      const leader = this.agents().get(squad.leaderAgentId)
      if (leader === undefined) continue
      const expected = this.persistentImplementerChildId(root.id, squadId, squad.leaderAgentId, leader)
      if (expected !== agent.id) continue
      return { root, squadId, squad, leaderAgentId: squad.leaderAgentId, leader }
    }
    return undefined
  }

  protected isPersistentImplementer(agent: Agent): boolean {
    return this.persistentImplementerBinding(agent) !== undefined
  }

  /**
   * rc.2 compatibility shim for per-member reasoning effort.
   *
   * `@deepseek-ai/dsh-agent` 0.1.1-rc.2 does not declare reasoningEffort on
   * AgentOptions, while the loop already accepts it at the official
   * `agent/request` waterfall. In-process subagent creation preserves unknown
   * own properties at runtime, so fresh Agent Team children carry the configured
   * effort here. A cold-resumed persistent Implementer reconstructs only the
   * rc.2 descriptor fields, so its Team binding is the durable fallback.
   */
  protected configuredReasoningEffort(agent: Agent): ReturnType<typeof ReasoningEffortId> | undefined {
    const transient = (agent.options as Agent['options'] & {
      readonly reasoningEffort?: ReturnType<typeof ReasoningEffortId>
    }).reasoningEffort
    if (transient !== undefined) return transient
    const binding = this.persistentImplementerBinding(agent)
    return binding?.leader.reasoningEffort === undefined
      ? undefined
      : ReasoningEffortId(binding.leader.reasoningEffort)
  }

  /** Install the rc.2 request-time bridge that makes configured child effort effective. */
  protected registerReasoningEffortRouting(): void {
    // Continuable activations use an independent owner scope, not this plugin's
    // scope. Install before publication on both fresh creation and cold resume.
    this.ctx.subagents.registerContinuableSetup(childCtx => {
      const child = childCtx.agent
      if (child === undefined) throw new AgentTeamError('continuable setup requires an Agent scope', 'INVALID_DISPATCH')
      const binding = this.persistentImplementerBinding(child)
      if (binding === undefined) return () => {}
      const unregisterTool = childCtx.tools.register(createDispatchPostmanBridgeTool(this))
      const unregisterRequest = childCtx.on('agent/request', async ({ agent }, next) => {
        const config = await next()
        if (agent !== childCtx.agent) return config
        const current = this.persistentImplementerBinding(agent)
        if (current === undefined) throw new AgentTeamError('Implementer definition changed; start a new Team conversation turn', 'INVALID_DISPATCH')
        return {
          ...config,
          provider: current.leader.provider,
          model: current.leader.model,
          ...(current.leader.maxTokens === undefined ? {} : { maxTokens: current.leader.maxTokens }),
          ...(current.leader.reasoningEffort === undefined ? {} : { reasoningEffort: ReasoningEffortId(current.leader.reasoningEffort) }),
        }
      })
      return () => { unregisterRequest(); unregisterTool() }
    })
    this.ctx.on('agent/request', async ({ agent }, next) => {
      const config = await next()
      const reasoningEffort = this.configuredReasoningEffort(agent)
      return reasoningEffort === undefined || config.reasoningEffort === reasoningEffort
        ? config
        : { ...config, reasoningEffort }
    })
  }

  /** Tool boundary for the dedicated Implementer: implementation tools survive, recursive orchestration does not, Bridge always does. */
  private persistentImplementerToolScope(record: AgentRecord, root: Agent): AgentRecord['toolScope'] {
    const schemas = this.ctx.tools.schemas(root)
    const known = new Set(schemas.map(tool => tool.name))
    if (!known.has('dispatch_postman_bridge')) {
      throw new AgentTeamError('persistent Implementer requires dispatch_postman_bridge in the parent tool scope', 'INVALID_DISPATCH')
    }
    const unknownAllowed = (record.toolScope?.allow ?? []).filter(name => !known.has(name))
    if (unknownAllowed.length > 0) {
      throw new AgentTeamError(`Implementer tool allow-list is unavailable in this conversation: ${unknownAllowed.join(', ')}`, 'INVALID_DISPATCH')
    }
    const deny = new Set((record.toolScope?.deny ?? []).filter(name => known.has(name)))
    if (deny.has('dispatch_postman_bridge')) {
      throw new AgentTeamError('persistent Implementer cannot deny dispatch_postman_bridge', 'INVALID_DISPATCH')
    }
    for (const name of ['dispatch_to_squad', 'subagent', 'workflow']) if (known.has(name)) deny.add(name)
    ...
    const configuredAllow = record.toolScope?.allow
    const allow = configuredAllow === undefined
      ? undefined
      : [...new Set([...configuredAllow, 'dispatch_postman_bridge'])]
    if (allow !== undefined) {
      const forbidden = allow.filter(name => deny.has(name))
      if (forbidden.length > 0) {
        throw new AgentTeamError(`Implementer allow-list cannot expose recursive orchestration tools: ${forbidden.join(', ')}`, 'INVALID_DISPATCH')
      }
    }
    return {
  ...
  private persistentImplementerPersona(squad: SquadRecord, leader: AgentRecord): string {
    return [
      '<agent_team_persistent_implementer>',
      `You are the dedicated Implementer for Team "${squad.name}". You live in your own continuable child session; the root conversation Agent is only a relay shell.`,
      'Authoritative workflow:',
      '1. For any task that requires architecture, coding, editing, or fixing implementation, do NOT invent the solution or write implementation from scratch. The external ChatGPT reached through Postman is the solution author.',
      '2. Send the exact current task through dispatch_postman_bridge. Do not ask the Bridge to reason, design, code, apply files, run Git, or test.',
      '3. After exact RESULT_DURABLE, inspect the returned artifact locally. Before any repository-changing application or Git operation, read and obey the current workspace REPO_POLICY.md and system/implementation-package-workflow.md. Apply exactly the prepared implementation in the workspace/location those current rules require, then run only the technical checks needed to verify it.',
      '4. Do not hard-code a branch/worktree/publication policy from this persona: current workspace repository policy is authoritative and may evolve independently of this plugin.',
      '5. If the external result is incomplete, incompatible with the current repository state, or a check exposes a problem in that result, do not repair the implementation from scratch. Call dispatch_postman_bridge again with chatRequestId equal to the previous REQ and send concise failure evidence so the same external ChatGPT conversation can revise the artifact.',
      '6. You may repeat that Bridge continuation loop serially as needed. Never start another squad, generic subagent, workflow, or recursive agent team.',
      '7. For purely factual/status questions that require no implementation, answer directly without manufacturing repository changes.',
      '8. Finish with a concise user-facing implementation report. The root shell will relay your final output without reinterpretation.',
      `Configured Implementer role prompt (additional operator instructions):\n${this.boundedExcerpt(leader.systemPrompt, 12_000)}`,
      '</agent_team_persistent_implementer>',
    ].join('\n\n')
  }

  private assistantTextAfterMessage(agent: Agent, messageId: string): string | undefined {
    let accepted = false
    let latest: string | undefined
    for (const event of agent.session.events) {
      if (event.type === 'user/message') {
        if (String(event.data.id) === messageId) {
          accepted = true
          latest = undefined
          continue
        }
      ...
      if (!accepted || event.type !== 'assistant/message') continue
      const text = this.resultText(event.data.message.content)
      if (text.trim() !== '') latest = text
    }
    return latest
  }

  private isNotResumable(error: unknown): boolean {
    return typeof error === 'object' && error !== null && 'code' in error
      && (error as { readonly code?: unknown }).code === 'NOT_RESUMABLE'
  }

  /** Route one top-level user task to the Team's durable dedicated Implementer and wait for that exact turn. */
  protected async dispatchPersistentImplementerConversation(
    squadId: SquadId,
    task: string,
    parent: Agent,
    signal: AbortSignal,
  ): Promise<PersistentImplementerConversationResult> {
    const squad = this.squads().get(squadId)
    if (squad === undefined) throw new AgentTeamError(`squad "${squadId}" does not exist`, 'SQUAD_NOT_FOUND')
    if (squad.conversationMode !== 'persistent-lead' || squad.leaderAgentId === undefined) {
      throw new AgentTeamError('selected squad is not configured for persistent-lead conversation routing', 'INVALID_DISPATCH')
    }
    const leader = this.agents().get(squad.leaderAgentId)
    if (leader === undefined) throw new AgentTeamError('persistent Implementer definition is missing', 'AGENT_NOT_FOUND')
    if (leader.kind === 'postman-bridge') throw new AgentTeamError('Postman Bridge cannot be the persistent Implementer', 'INVALID_DISPATCH')
    const childId = this.persistentImplementerChildId(parent.id, squadId, squad.leaderAgentId, leader)
    const content: ContentBlock[] = [{ type: 'text', text: task }]
    let messageId: string
    try {
      messageId = String(await this.ctx.subagents.followup(parent, childId, content, {
        source: { kind: 'coordinator', form: 'relay', senderSessionId: parent.id },
        signal,
      }))
    } catch (error: unknown) {
      if (!this.isNotResumable(error)) throw error
      const toolFilter = this.persistentImplementerToolScope(leader, parent)
      const started = await this.ctx.subagents.startContinuable({
        childId,
        provider: 'spawn',
        label: `Agent Team/${squad.name}/${leader.name}`,
        request: {
          prompt: content,
          parent,
          agentOptions: {
            ...
            model: leader.model,
            ...(leader.reasoningEffort === undefined ? {} : { reasoningEffort: ReasoningEffortId(leader.reasoningEffort) }),
            ...(leader.maxTokens === undefined ? {} : { maxTokens: leader.maxTokens }),
          },
          ...(toolFilter === undefined ? {} : { toolFilter }),
          persona: this.persistentImplementerPersona(squad, leader),
    ...
    if (child === undefined) throw new AgentTeamError('persistent Implementer was accepted but is not live', 'INVALID_DISPATCH')
    const onAbort = (): void => {
    ...
    const output = this.assistantTextAfterMessage(child, messageId)
    if (output === undefined || output.trim() === '') {
      throw new AgentTeamError('persistent Implementer completed without a non-empty assistant result for this task', 'INVALID_DISPATCH')
    }
    return {
      childId,
      messageId,
  ...
  private resolvePostmanBridgeForCaller(reference: string | undefined, parent: Agent): [AgentId, AgentRecord] {
    const binding = this.persistentImplementerBinding(parent)
    if (binding !== undefined) {
      const candidates = binding.squad.members.flatMap((id) => {
        const record = this.agents().get(id)
        return record?.kind === 'postman-bridge' ? [[id, record] as [AgentId, AgentRecord]] : []
      })
      if (reference !== undefined) {
        const found = candidates.find(([id, record]) => String(id) === reference || record.name === reference)
        if (found === undefined) throw new AgentTeamError('requested Postman Bridge is not a Bridge member of this persistent Team', 'INVALID_DISPATCH')
        return found
      }
      if (candidates.length === 1) return candidates[0]!
      if (candidates.length === 0) throw new AgentTeamError('persistent Team has no Postman Bridge member', 'AGENT_NOT_FOUND')
      throw new AgentTeamError('persistent Team has multiple Postman Bridge members; specify bridgeAgent explicitly', 'INVALID_DISPATCH')
    }
    if (this.isDelegatedAgent(parent)) {
      throw new AgentTeamError('delegated agents cannot dispatch the Postman Bridge unless they are an authorized persistent Implementer', 'INVALID_DISPATCH')
    }
    const active = this.getEffectiveSessionSquadMode(parent)
    if (active !== undefined && this.squads().get(active.squadId)?.conversationMode === 'persistent-lead') {
      throw new AgentTeamError('the root conversation is a relay shell; its persistent Implementer owns Postman dispatch', 'INVALID_DISPATCH')
    }
    return this.resolvePostmanBridgeAgent(reference)
  }

  async dispatchPostmanBridgeFromTool(
    request: PostmanBridgeDispatchRequest,
    parent: Agent,
    signal: AbortSignal,
  ): Promise<PostmanBridgeDispatchResult> {
    const task = request.task
    if (task.trim() === '') throw new AgentTeamError('Postman Bridge task must not be empty', 'INVALID_DISPATCH')
    if (task.length > 100_000) throw new AgentTeamError('Postman Bridge task exceeds 100000 characters', 'INVALID_DISPATCH')
    if (request.chatRequestId !== undefined && !/^REQ_\d{8}T\d{6}Z_\d{4}$/.test(request.chatRequestId)) {
      throw new AgentTeamError('chatRequestId must be a canonical REQ_YYYYMMDDTHHMMSSZ_NNNN value', 'INVALID_DISPATCH')
    }

    const [bridgeAgentId, record] = this.resolvePostmanBridgeForCaller(request.bridgeAgent, parent)
    const provider = 'spawn'
    const runtime = this.ctx.subagents as unknown as {
      getProvider?(name: string): { readonly capabilities: {
    ...
    if (capabilities === undefined || capabilities.agentOptions === false || capabilities.toolFilter === false
      || capabilities.depthLimit === false || capabilities.persona === false) {
      throw new AgentTeamError('spawn subagent provider cannot enforce the Postman Bridge capability boundary', 'INVALID_DISPATCH')
    }

    const internalToolName = `postman_direct_bridge_${randomUUID().replace(/-/g, '').slice(0, 20)}`
    let captured: DirectPostmanBridgeResult | undefined
    const unregister = this.ctx.tools.register(createPostmanDirectCapabilityTool({
      name: internalToolName,
      parent,
      task,
      ...(request.chatRequestId === undefined ? {} : { chatRequestId: request.chatRequestId }),
      invoke: this.postmanInvoker,
    ...
    try {
      const bridgePersona = [
        'You are the Postman Bridge. You are transport-only.',
        'Never design the solution, never write implementation code, never modify files or Git, and never call or create other agents.',
        `Call the one available internal tool ${internalToolName} exactly once. The host already bound the exact task and optional chat reference to that capability; do not rewrite or summarize the task.`,
        'After the tool settles, return only its JSON result. Do not inspect the ZIP contents and do not attempt local implementation.',
        `Operator note:\n${this.boundedExcerpt(record.systemPrompt, 8_000)}`,
      ].join('\n\n')
      run = await this.ctx.subagents.start(provider, {
        label: `Postman Bridge/${record.name}`,
        prompt: [{ type: 'text', text: `Execute the bound Postman transport now by calling ${internalToolName} exactly once.` }],
        parent,
        signal,
        agentOptions: {
          provider: record.provider,
          model: record.model,
          ...(record.reasoningEffort === undefined ? {} : { reasoningEffort: ReasoningEffortId(record.reasoningEffort) }),
          ...(record.maxTokens === undefined ? {} : { maxTokens: record.maxTokens }),
        },
        toolFilter: { allow: [internalToolName] },
      ...
      try {
        ...
        if (captured === undefined) {
          throw new AgentTeamError(`Postman Bridge child failed before transport completion: ${this.errorText(error)}`, 'INVALID_DISPATCH')
        }
      } finally {
        try {
          await run.dispose()
          ...
          if (captured === undefined) {
            throw new AgentTeamError(`Postman Bridge child cleanup failed before transport completion: ${this.errorText(error)}`, 'INVALID_DISPATCH')
          }
          this.ctx.logger.warn(`[agent-team-gui] Postman Bridge child cleanup warning: ${this.errorText(error)}`)
        }
      }
      if (captured === undefined) {
        throw new AgentTeamError('Postman Bridge child completed without invoking Direct Postman', 'INVALID_DISPATCH')
      }
      return {
        ...captured,
        bridgeAgentId,
        bridgeAgentName: record.name,
        childId: String(run.localAgent?.id ?? run.id),
        provider: record.provider,
        model: record.model,
  ...
  private async runMember(
    ...
    const selectedRoute = route ?? {
      ...
      ...(member.record.reasoningEffort === undefined ? {} : { reasoningEffort: member.record.reasoningEffort }),
    }
    ...
    try {
      const run = await this.ctx.subagents.start(provider, {
        ...
        agentOptions: {
          provider: selectedRoute.provider,
          model: selectedRoute.model,
          ...(selectedRoute.reasoningEffort === undefined ? {} : { reasoningEffort: ReasoningEffortId(selectedRoute.reasoningEffort) }),
  ...
  private childToolScope(record: AgentRecord, provider: string, parent: Agent): AgentRecord['toolScope'] | undefined {
    ...
    for (const name of ['dispatch_to_squad', 'dispatch_postman_bridge', 'subagent', 'workflow']) {
  ...
  private async runMemberWithPolicy(
    ...
    const fallback = member.record.fallbackProvider !== undefined && member.record.fallbackModel !== undefined
      ? {
          ...
          ...(member.record.fallbackReasoningEffort === undefined ? {} : { reasoningEffort: member.record.fallbackReasoningEffort }),
        }
      : undefined
  ...
  private async createAutomaticPlan(
    ...
    const plannerReasoningEffort = useMainAgent
      ? parent.session.requestHeader()?.config.reasoningEffort
      : leader?.reasoningEffort === undefined ? undefined : ReasoningEffortId(leader.reasoningEffort)
    const plannerAgentOptions = {
      ...(plannerProvider === undefined ? {} : { provider: plannerProvider }),
      ...(plannerModel === undefined ? {} : { model: plannerModel }),
      ...(plannerReasoningEffort === undefined ? {} : { reasoningEffort: plannerReasoningEffort }),
    ...
    try {
      run = await this.ctx.subagents.start(provider, {
        ...
        agentOptions: {
          ...
          ...(reviewer.record.reasoningEffort === undefined ? {} : { reasoningEffort: ReasoningEffortId(reviewer.record.reasoningEffort) }),
