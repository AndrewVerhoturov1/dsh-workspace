...
export class DefinitionApplicationService extends Service {
  ...
  protected async validateModelRoute(
    ...
    if (record.fallbackReasoningEffort !== undefined && (record.fallbackProvider === undefined || record.fallbackModel === undefined)) {
      throw new Error('fallbackReasoningEffort requires a configured fallback provider and model')
    }
    try {
      const primary = await abortableStep(signal, () => this.ctx.llm.resolveModelInfo(record.provider, record.model, signal))
      if (record.reasoningEffort !== undefined
        && !primary.reasoning?.efforts.some(effort => effort.id === record.reasoningEffort)) {
        throw new Error(`reasoning effort "${record.reasoningEffort}" is not supported by ${record.provider}/${record.model}`)
      }
      if (record.fallbackProvider !== undefined && record.fallbackModel !== undefined) {
        const fallback = await abortableStep(signal, () => this.ctx.llm.resolveModelInfo(record.fallbackProvider!, record.fallbackModel!, signal))
        if (record.fallbackReasoningEffort !== undefined
          && !fallback.reasoning?.efforts.some(effort => effort.id === record.fallbackReasoningEffort)) {
          throw new Error(`reasoning effort "${record.fallbackReasoningEffort}" is not supported by ${record.fallbackProvider}/${record.fallbackModel}`)
        }
      }
    } catch (error: unknown) {
  ...
  protected validateSquadRecord(
    ...
    if (record.conversationMode === 'persistent-lead') {
      if (record.leaderAgentId === undefined) {
        throw new AgentTeamError('persistent-lead conversation mode requires leaderAgentId', 'INVALID_MEMBERS')
      }
      if (record.members.length !== 2) {
        throw new AgentTeamError('persistent-lead mode requires exactly two members: one Implementer leader and one Postman Bridge', 'INVALID_MEMBERS')
      }
      const resolvedMembers = record.members.map((id) => this.agents().get(id))
      if (resolvedMembers.every((item): item is AgentRecord => item !== undefined)) {
        const leader = this.agents().get(record.leaderAgentId)
        if (leader?.kind === 'postman-bridge') {
          throw new AgentTeamError('persistent-lead leader cannot be a Postman Bridge member', 'INVALID_MEMBERS')
        }
        const bridgeMembers = resolvedMembers.filter(member => member.kind === 'postman-bridge')
        if (bridgeMembers.length !== 1) {
          throw new AgentTeamError('persistent-lead mode requires exactly one Postman Bridge member', 'INVALID_MEMBERS')
        }
        if (leader?.toolScope?.deny?.includes('dispatch_postman_bridge') === true) {
          throw new AgentTeamError('persistent-lead Implementer cannot deny dispatch_postman_bridge', 'INVALID_DISPATCH')
        }
      }
      if ((record.responseMode ?? 'foreground') !== 'foreground') {
        throw new AgentTeamError('persistent-lead mode requires foreground responseMode', 'INVALID_DISPATCH')
      }
      if ((record.triggerMode ?? 'guaranteed') !== 'guaranteed') {
        throw new AgentTeamError('persistent-lead mode requires guaranteed triggerMode', 'INVALID_DISPATCH')
      }
      if ((record.activationMode ?? 'always') !== 'always') {
        throw new AgentTeamError('persistent-lead mode requires always activation so the root conversation remains a pure relay shell', 'INVALID_DISPATCH')
      }
    }
  ...
  squadModeGuidance(agent: Agent | undefined): string {
    ...
    if (squad.conversationMode === 'persistent-lead') {
      ...
      return [
        '<agent_team_persistent_lead_shell>',
        `Dedicated Team mode is active for this conversation. Team id: ${mode.squadId}.`,
        `Team name: ${squad.name}. Dedicated Implementer: ${leader?.name ?? squad.leaderAgentId ?? '(missing)'}.`,
        'This root conversation Agent is only the user-facing shell. It is NOT the Implementer and must not design, code, edit files, run tests, or call Postman on behalf of the team.',
        'Before the root model is allowed to answer, the host routes the exact user request to the Team\'s persistent Implementer child and injects one authoritative IMPLEMENTER_RESULT notice.',
        'When that notice is present, return its IMPLEMENTER_OUTPUT text exactly, preserving Markdown and paths. Do not reinterpret, summarize, improve, critique, or add your own solution.',
        'If the notice reports IMPLEMENTER_FAILED, report only that failure briefly. Do not fall back to solving the task yourself.',
        '</agent_team_persistent_lead_shell>',
      ].join('\n')
    }
  ...
  resolveAgentId(reference: string): AgentId {
    const trimmed = reference.trim()
    const direct = AgentId(trimmed)
    if (this.agents().get(direct) !== undefined) return direct
    const matches = this.listAgents().filter(([, agent]) => agent.name.trim().toLocaleLowerCase() === trimmed.toLocaleLowerCase())
    if (matches.length === 1) return matches[0]![0]
    if (matches.length > 1) throw new AgentTeamError(`agent name "${trimmed}" is ambiguous; use its durable id`, 'INVALID_DISPATCH')
    throw new AgentTeamError(`agent "${trimmed}" does not exist`, 'AGENT_NOT_FOUND')
  }

  resolvePostmanBridgeAgent(reference?: string): [AgentId, AgentRecord] {
    if (reference !== undefined && reference.trim() !== '') {
      const id = this.resolveAgentId(reference)
      const record = this.agents().get(id)
      if (record === undefined) throw new AgentTeamError(`agent "${reference}" does not exist`, 'AGENT_NOT_FOUND')
      if ((record.kind ?? 'standard') !== 'postman-bridge') {
        throw new AgentTeamError(`agent "${record.name}" is not marked as a Postman Bridge`, 'INVALID_DISPATCH')
      }
      return [id, record]
    }
    const bridges = this.listAgents().filter(([, record]) => record.kind === 'postman-bridge')
    if (bridges.length === 1) return bridges[0]!
    if (bridges.length === 0) throw new AgentTeamError('no agent is marked as a Postman Bridge', 'AGENT_NOT_FOUND')
    throw new AgentTeamError('multiple agents are marked as Postman Bridge; specify bridgeAgent explicitly', 'INVALID_DISPATCH')
  }
