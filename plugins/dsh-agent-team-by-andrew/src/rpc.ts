...
const agentInputSchema = z.object({
  ...
  fallbackReasoningEffort: z.string().trim().min(1).max(200).optional(),
  kind: z.enum(['standard', 'postman-bridge']).optional(),
}).strict()
const squadInputSchema = z.object({
  ...
  conversationMode: z.enum(['classic', 'persistent-lead']).optional(),
...
function agentRecord(input: AgentInput) {
  return {
    ...
    ...(input.fallbackReasoningEffort === undefined ? {} : { fallbackReasoningEffort: input.fallbackReasoningEffort }),
...
function squadRecord(input: SquadInput) {
  return {
    ...
    ...(input.conversationMode === undefined ? {} : { conversationMode: input.conversationMode }),
...
const agentSnapshotSchema = agentInputSchema.extend({
  ...
  fallbackReasoningEffort: z.string().trim().min(1).optional(),
  kind: z.enum(['standard', 'postman-bridge']).optional(),
})
