...
export interface AgentRecord {
  ...
  /** Adapter-owned reasoning effort for this exact provider/model route. */
  readonly reasoningEffort?: string
  readonly maxTokens?: number
  /** Optional specialized role used by direct host-owned dispatchers. */
  readonly kind?: 'standard' | 'postman-bridge'
...
/** One request from the main Implementer to the dedicated Postman Bridge child. */
export interface PostmanBridgeDispatchRequest {
  readonly bridgeAgent?: string
  readonly task: string
  readonly chatRequestId?: string
}

/** Structured transport-only handoff returned to the main Implementer. */
export interface PostmanBridgeDispatchResult {
  readonly ok: boolean
  ...
  readonly bridgeAgentId: AgentId
...
export interface SquadRecord {
  ...
  /** Conversation routing. `persistent-lead` makes the selected leader a durable dedicated Implementer child. */
  readonly conversationMode?: 'classic' | 'persistent-lead'
