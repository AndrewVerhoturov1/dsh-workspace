...
function TeamWorkspace({ controller, data, draft, baseline, busy, sessionId, locale, t, setDraft, setBaseline, run, setNotice, select, clone, remove, externalConflict, clearExternalConflict }: WorkspaceCommon & {
  ...
  return <div className="atg-workspace-shell">
    ...
    <main className="atg-editor" aria-label={t('teamEditor')}>
      ...
      <details className="atg-disclosure" open><summary><span><strong>{t('orchestration')}</strong><small>{t('autoPlan')}</small></span><span aria-hidden="true">⌄</span></summary><div className="atg-disclosure-body"><div className="atg-three"><SelectField label={t('activationMode')} value={draft.activationMode} options={[['always', t('always')], ['smart', t('smart')], ['manual', t('manual')]]} error={validation.errors.activationMode === undefined ? undefined : t(validation.errors.activationMode)} onChange={value => { setDraft(current => ({ ...current, activationMode: value as SquadDraft['activationMode'] })) }} /><SelectField label={t('memberSelection')} value={draft.memberSelectionMode} options={[['all', t('allMembers')], ['adaptive', t('adaptive')]]} onChange={value => { setDraft(current => ({ ...current, memberSelectionMode: value as SquadDraft['memberSelectionMode'] })) }} /><SelectField label={t('responseMode')} value={draft.responseMode} options={[['foreground', t('foreground')], ['background', t('background')]]} error={validation.errors.responseMode === undefined ? undefined : t(validation.errors.responseMode)} onChange={value => { setDraft(current => ({ ...current, responseMode: value as SquadDraft['responseMode'] })) }} /></div><div className="atg-three"><SelectField label={t('executionMode')} value={draft.executionMode} options={[['inherit', executionInheritLabel], ['serial', t('serial')], ['parallel', t('parallel')]]} disabled={draft.fixedOrder} onChange={value => { setDraft(current => ({ ...current, executionMode: value as SquadDraft['executionMode'] })) }} /><SelectField label={t('contextMode')} value={draft.contextMode} options={[['inherit', contextInheritLabel], ['spawn', t('spawn')], ['fork', t('fork')], ['chain', t('chain')]]} error={validation.errors.contextMode === undefined ? undefined : t(validation.errors.contextMode)} onChange={value => { setDraft(current => ({ ...current, contextMode: value as SquadDraft['contextMode'] })) }} /><SelectField label={t('planningContext')} value={draft.planningContext} options={[['inherit', t('legacyPlanningFull')], ['current', t('current')], ['recent', t('recent')], ['full', t('full')]]} onChange={value => { setDraft(current => ({ ...current, planningContext: value as SquadDraft['planningContext'] })) }} /></div><div className="atg-two"><SelectField label={t('conversationRouting')} value={draft.conversationMode} options={[['classic', t('classicConversation')], ['persistent-lead', t('persistentLeadConversation')]]} onChange={value => { setDraft(current => ({ ...current, conversationMode: value as SquadDraft['conversationMode'] })) }} /><SelectField label={draft.conversationMode === 'persistent-lead' ? t('persistentImplementer') : t('teamLeader')} value={draft.leaderAgentId} options={[['', t('noLeader')], ...data.agents.filter(item => draft.members.includes(item.id)).map(item => [item.id, item.name] as [string, string])]} error={validation.errors.leaderAgentId === undefined ? undefined : t(validation.errors.leaderAgentId)} onChange={value => { setDraft(current => ({ ...current, leaderAgentId: value })) }} /></div>{draft.conversationMode === 'persistent-lead' && <small className="atg-field-hint">{t('persistentLeadHint')}</small>}<div className="atg-two"><Field id="planner-limit" label={t('plannerMaxTokens')} value={draft.plannerMaxTokens} inputMode="numeric" error={validation.errors.plannerMaxTokens === undefined ? undefined : t(validation.errors.plannerMaxTokens)} onChange={value => { setDraft(current => ({ ...current, plannerMaxTokens: value })) }} /></div><label className="atg-toggle-row"><input type="checkbox" checked={draft.fixedOrder} onChange={event => { setDraft(current => ({ ...current, fixedOrder: event.currentTarget.checked, executionMode: event.currentTarget.checked ? 'serial' : current.executionMode })) }} /><span><strong>{t('fixedOrder')}</strong><small>{t('fixedOrderHint')}</small></span></label>{draft.fixedOrder && <OrderEditor draft={draft} agents={data.agents} setDraft={setDraft} t={t} />}</div></details>
