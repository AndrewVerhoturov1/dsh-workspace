...
export interface SquadDraft {
  ...
  conversationMode: 'classic' | 'persistent-lead'
...
export const EMPTY_AGENT: AgentDraft = {
  id: '', name: '', systemPrompt: '', provider: '', model: '', reasoningEffort: '', maxTokens: '', kind: 'standard', allow: '', deny: '', fallbackProvider: '', fallbackModel: '', fallbackReasoningEffort: '',
}
...
export const EMPTY_SQUAD: SquadDraft = {
  ...
  leaderAgentId: '', conversationMode: 'classic', triggerMode: 'guaranteed', failurePolicy: 'continue', maxConcurrency: '', memberTimeoutMs: '', tokenBudget: '',
...
export function agentDraftOf(agent: AgentView): AgentDraft {
  return {
    ...
    fallbackReasoningEffort: agent.fallbackReasoningEffort ?? '',
...
export function squadDraftOf(squad: SquadView): SquadDraft {
  return {
    ...
    conversationMode: squad.conversationMode ?? 'classic',
...
export function validateAgent(draft: AgentDraft, tools?: readonly { name: string }[]): ValidationResult {
  ...
  if (draft.reasoningEffort.trim().length > 200) errors.reasoningEffort = 'routeLength'
  ...
  else if (draft.fallbackProvider.trim().length > 200 || draft.fallbackModel.trim().length > 200 || draft.fallbackReasoningEffort.trim().length > 200) errors.fallback = 'routeLength'
  if (draft.fallbackReasoningEffort !== '' && (draft.fallbackProvider === '' || draft.fallbackModel === '')) errors.fallback = 'fallbackPairError'
...
export function validateSquad(draft: SquadDraft, agents: readonly AgentView[], defaults?: TeamDefaults): ValidationResult {
  ...
  if (draft.conversationMode === 'persistent-lead') {
    const selected = agents.filter(agent => draft.members.includes(agent.id))
    const bridges = selected.filter(agent => agent.kind === 'postman-bridge')
    if (draft.members.length !== 2 || bridges.length !== 1) errors.members = 'persistentLeadTwoMemberError'
    if (draft.leaderAgentId === '' || !draft.members.includes(draft.leaderAgentId)) errors.leaderAgentId = 'persistentLeadRequired'
    else {
      const leader = agents.find(agent => agent.id === draft.leaderAgentId)
      if (leader?.kind === 'postman-bridge') errors.leaderAgentId = 'persistentLeadBridgeError'
      if (leader?.toolScope?.deny?.includes('dispatch_postman_bridge') === true) errors.leaderAgentId = 'persistentLeadToolError'
    }
    if (draft.responseMode === 'background') errors.responseMode = 'persistentLeadForegroundOnly'
    if (draft.triggerMode !== 'guaranteed') errors.triggerMode = 'persistentLeadGuaranteedOnly'
    if (draft.activationMode !== 'always') errors.activationMode = 'persistentLeadAlwaysOnly'
  }
...
export function toAgentRecord(draft: AgentDraft): Omit<AgentView, 'id'> {
  ...
  return {
    ...
    ...(draft.fallbackProvider === '' || draft.fallbackModel === '' ? {} : {
      ...
      ...(draft.fallbackReasoningEffort.trim() === '' ? {} : { fallbackReasoningEffort: draft.fallbackReasoningEffort.trim() }),
    }),
    ...(draft.kind === 'standard' ? {} : { kind: draft.kind }),
  }
}
...
export function toSquadRecord(draft: SquadDraft): Omit<SquadView, 'id'> {
  return {
    ...
    ...(draft.conversationMode === 'classic' ? {} : { conversationMode: draft.conversationMode }),
