...
export class AgentTeamController {
  ...
  constructor(call: AgentTeamRpc, locale?: LocaleService, forceRussian = false) {
...
function isAgentSummary(value: unknown): boolean {
  ...
  const record = value as Record<string, unknown>
  if (!isOptionalNumber(record.maxTokens)
    || !isOptionalString(record.reasoningEffort)
    || !isOptionalString(record.fallbackProvider)
    || !isOptionalString(record.fallbackModel)
    || !isOptionalString(record.fallbackReasoningEffort)) return false
  if (record.kind !== undefined && record.kind !== 'standard' && record.kind !== 'postman-bridge') return false
