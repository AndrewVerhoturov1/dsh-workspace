...
import type { BranchRuntimeService } from './runtime/runtime-service.ts'
...
export function registerWorktreeStudioWeb(ctx: Context, github: GitHubClient, runtime: BranchRuntimeService): () => void {
  return ctx.webServer.register({
    ...
    async handler(request, response) {
      ...
      try {
        ...
        const value = await dispatch(ctx, github, runtime, await readBody(request))
...
async function dispatch(ctx: Context, github: GitHubClient, runtime: BranchRuntimeService, input: Record<string, unknown>): Promise<unknown> {
  const operation = requiredString(input, 'operation')
  if (operation === 'runtime.inspect') return runtime.inspectExternal(requiredString(input, 'worktreePath'))
  if (operation === 'runtime.startExternal') return await runtime.startExternal(requiredString(input, 'worktreePath'))
  if (operation === 'runtime.status') return runtime.statusByWorktree(requiredString(input, 'worktreePath'))
  if (operation === 'runtime.stop') return await runtime.stop(requiredString(input, 'runtimeId'))
  if (operation === 'runtime.cleanup') return runtime.cleanup(requiredString(input, 'runtimeId'))
  if (operation === 'runtime.start') {
    const id = taskId(input)
    const changeToken = requiredToken(input)
    const inspected = await ctx.worktreeStudio.inspect(id)
    if (inspected.task.changeToken !== changeToken) {
      throw new StudioError('state-conflict', 'task changed before runtime launch')
    }
    return await runtime.startTask(inspected.task)
  }
  if (operation === 'create') {
