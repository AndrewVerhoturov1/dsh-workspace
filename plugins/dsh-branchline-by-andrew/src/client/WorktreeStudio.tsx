...
import type { BranchRuntimeView } from '../runtime/types.ts'
...
function TaskBoard({
  ...
  const runtimeEnabled = typeof runtimeStatus === 'function'
  ...
  const [runtime, setRuntime] = useState<BranchRuntimeView | null>(null)
  ...
  useEffect(() => {
    if (!open || !runtimeEnabled || selected === undefined || !selected.exists) {
      setRuntime(null)
      return
    }
    let cancelled = false
    void runtimeStatus(selected.workspacePath).then(
      value => { if (!cancelled) setRuntime(value) },
      () => { if (!cancelled) setRuntime(null) },
    )
    return () => { cancelled = true }
  }, [open, refresh, runtimeEnabled, runtimeStatus, selected?.workspacePath])
        ...
        <main className={css.content}>
          ...
          {createOpen ? (
            ...
            <TaskDetails
              ...
              onRuntimeStart={() => {
                void run('runtime-start', () => startTaskRuntime(selected), setRuntime)
              }}
              onRuntimeStop={() => {
                if (runtime !== null) void run('runtime-stop', () => stopRuntime(runtime.runtimeId), setRuntime)
              }}
              onRuntimeCleanup={() => {
                if (runtime === null) return
                void run('runtime-cleanup', async () => {
                  await cleanupRuntime(runtime.runtimeId)
                  return null
                }, setRuntime)
              }}
              onRuntimeOpen={() => {
                if (runtime?.authenticatedUrl !== undefined) {
                  window.open(runtime.authenticatedUrl, '_blank', 'noopener,noreferrer')
                }
              }}
            />
          )}
...
function TaskDetails({
  ...
  readonly runtime: BranchRuntimeView | null
  ...
  readonly onRuntimeCleanup: () => void
  ...
  return (
    <div className={css.details}>
      ...
      <div className={css.workflowBar}>
        ...
        {runtimeEnabled && (runtime === null || runtime.state === 'STOPPED' || runtime.state === 'ERROR') ? (
          <Button size="sm" variant="outline" icon={<IconPlayOutline16 size={14} />} disabled={busy !== undefined || !task.exists} onClick={onRuntimeStart}>
            {t('testRuntime')}
          </Button>
        ) : null}
        {runtimeEnabled && runtime?.authenticatedUrl !== undefined && (runtime.state === 'RUNNING' || runtime.state === 'STALE_SOURCE') ? (
          <Button size="sm" variant="outline" disabled={busy !== undefined} onClick={onRuntimeOpen}>{t('runtimeOpen')}</Button>
        ) : null}
        {runtimeEnabled && runtime !== null && (runtime.state === 'RUNNING' || runtime.state === 'STALE_SOURCE') ? (
          <Button size="sm" variant="outline" disabled={busy !== undefined} onClick={onRuntimeStop}>{t('runtimeStop')}</Button>
        ) : null}
        {runtimeEnabled && runtime !== null && (runtime.state === 'STOPPED' || runtime.state === 'ERROR') ? (
          <Button size="sm" variant="outline" disabled={busy !== undefined} onClick={onRuntimeCleanup}>{t('runtimeCleanup')}</Button>
        ) : null}
      ...
      <div className={css.resultBands}>
        {runtimeEnabled && runtime !== null ? (
          <StatusBand
            state={runtime.state === 'RUNNING' ? 'success' : runtime.state === 'STALE_SOURCE' || runtime.state === 'ERROR' ? 'error' : 'neutral'}
            title={t('testRuntime')}
            text={`${runtime.state}${runtime.port === undefined ? '' : ` :${String(runtime.port)}`}${runtime.overrides.length === 0 ? '' : ` · ${runtime.overrides.join(', ')}`}`}
          />
        ) : null}
        <StatusBand
