...
export function apply(ctx: ClientContext): void {
  ...
  const actions: StudioClientActions = {
    ...
    startTaskRuntime: task => post({ operation: 'runtime.start', id: task.id, changeToken: task.changeToken }),
    runtimeStatus: worktreePath => post({ operation: 'runtime.status', worktreePath }),
    stopRuntime: runtimeId => post({ operation: 'runtime.stop', runtimeId }),
    async cleanupRuntime(runtimeId) {
      await post({ operation: 'runtime.cleanup', runtimeId })
    },
