...
export function GitView(props: {
  ...
  const refresh = useCallback(async (): Promise<void> => {
    ...
    try {
      const [statusResult, branchResult, logResult] = await Promise.all([
        ...
        api.gitBranch(operationScope).catch(() => ({ current: '', names: [] as string[] })),
        // The first history page only; the rest arrives via "load more".
        api.gitLog(operationScope, LOG_BATCH, 0).catch(() => [] as GitLogEntry[]),
      ])
  ...
  }, [scope.sessionId, scope.cwd, workspaceRoot])
  ...
  return (
    <div className={css.git}>
      <div className={css.gitHeader}>
        <WorkspaceTargetSelect scope={{ sessionId: scope.sessionId, cwd: scope.cwd }} store={store} />
        <select
