...
export function DiffTab(props: { sessionId: string; cwd: string | undefined; workspaceRoot?: string; diff: SidebarDiffRef }) {
  ...
  useEffect(() => {
    let cancelled = false
    const scope: SessionScope = { sessionId, cwd, ...(workspaceRoot === undefined ? {} : { workspaceRoot }) }
    setLoading(true)
    setError(null)
    setData(null)
  ...
  }, [sessionId, cwd, workspaceRoot, diff, tick])
