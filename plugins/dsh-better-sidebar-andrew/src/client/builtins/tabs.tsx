    ...
    },
    {
      ...
      component: ({ ctx, store, scope, onOpenDiff }) => (
        <GitView
          scope={scope}
          store={store}
          onOpenFile={(path, workspaceRoot) => { openSidebarFile(ctx, store, scope.sessionId, path, workspaceRoot) }}
      ...
      component: ({ scope, tab }) => (
        tab.diff === undefined ? null
          : <DiffTab
              sessionId={scope.sessionId}
              cwd={scope.cwd}
              workspaceRoot={typeof tab.meta === 'object' && tab.meta !== null && typeof (tab.meta as Record<string, unknown>).workspaceRoot === 'string'
                ? (tab.meta as Record<string, unknown>).workspaceRoot as string
                : undefined}
              diff={tab.diff}
            />
      ),
