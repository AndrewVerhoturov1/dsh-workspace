...
/** One physical linked worktree returned by the host selector route. */
...
async function fetchUpload<T>(
  ...
  if (scope.workspaceRoot !== undefined && scope.workspaceRoot !== '') params.set('workspaceRoot', scope.workspaceRoot)
  let response: Response
  try {
...
export interface SessionScope {
  ...
  cwd?: string
  /** Selected physical worktree for built-in Files/Git surfaces only. */
...
function scopePayload(scope: SessionScope, extra: Record<string, unknown>): Record<string, unknown> {
  return {
    ...
    ...(scope.workspaceRoot !== undefined && scope.workspaceRoot !== '' ? { workspaceRoot: scope.workspaceRoot } : {}),
    ...extra,
  }
}
