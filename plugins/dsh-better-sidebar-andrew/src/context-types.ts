...
export type SidebarPresentationKind = 'html' | 'url' | 'file' | 'folder'

export interface SidebarPresentationRequest {
  sessionId: string
  workspaceRoot?: string
  kind: SidebarPresentationKind
  target: string
  title?: string
}

export type SidebarPresentationWireRequest =
  | ({ id: string; action: 'present' } & SidebarPresentationRequest)
  | { id: string; action: 'clear'; sessionId: string; workspaceRoot?: string }

/** Host-only deterministic bridge for user-visible result presentation. */
export interface BetterSidebarPresentationService {
  present(input: SidebarPresentationRequest): Promise<{
    id: string
    delivered: boolean
    workspaceRoot?: string
    target: string
  }>
  clear(input: { sessionId: string; workspaceRoot?: string }): Promise<{
    id: string
    delivered: boolean
    workspaceRoot?: string
  }>
...
declare module 'cordis' {
  interface Context {
    ...
    betterSidebarPresentation: BetterSidebarPresentationService
    /**
