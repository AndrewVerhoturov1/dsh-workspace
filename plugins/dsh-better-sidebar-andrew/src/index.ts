...
import { createBetterSidebarPresentationService, SidebarPresentationRegistry } from './presentation.ts'
import { ensureWorkspacePath } from './path-security.ts'
import { listWorkspaceTargets, resolveWorkspaceCwd } from './workspace-root.ts'
...
async function resolveGitPath(cwd: string, raw: string): Promise<string> {
  ...
  // Installed Better Sidebar is also tested/loaded as a nested package inside
  // the DSH repository. Prefer an existing path relative to the caller's
  // workspace when it is present; otherwise use Git's repository-top-level
  // spelling for status entries and deleted files.
...
function buildApi(
  ...
  const cwdOf = async (payload: unknown): Promise<{ sessionId: string; cwd: string; workspaceRoot?: string }> => {
    const sessionId = requireString(payload, 'sessionId')
    const record = payload as { cwd?: unknown; workspaceRoot?: unknown } | null
    const clientCwd = typeof record?.cwd === 'string' && record.cwd !== '' ? record.cwd : undefined
    const base = sessionCwdOf(ctx, sessionId, clientCwd)
    const requested = typeof record?.workspaceRoot === 'string' && record.workspaceRoot !== '' ? record.workspaceRoot : undefined
    const cwd = await resolveWorkspaceCwd(base, requested)
    return { sessionId, cwd, ...(requested === undefined ? {} : { workspaceRoot: cwd }) }
  }
  ...
  return {
    ...
    'fs.read': async (payload) => {
      ...
      const path = workspaceRoot === undefined ? resolvedPath : await ensureWorkspacePath(workspaceRoot, resolvedPath)
    ...
    'git.diff': async (payload) => {
      ...
      const resolvedPath = record.path === undefined ? undefined : await resolveGitPath(cwd, requireString(payload, 'path'))
      const path = resolvedPath === undefined || workspaceRoot === undefined
        ? resolvedPath
        : await ensureWorkspacePath(workspaceRoot, resolvedPath, true)
    ...
    'git.discard': async (payload) => {
      const { cwd, workspaceRoot } = await cwdOf(payload)
      const resolvedPath = await resolveGitPath(cwd, requireString(payload, 'path'))
      await git.discard(cwd, workspaceRoot === undefined ? resolvedPath : await ensureWorkspacePath(workspaceRoot, resolvedPath, true))
      return { ok: true }
    },
    ...
    'git.show': async (payload) => {
      const { cwd, workspaceRoot } = await cwdOf(payload)
      const resolvedPath = await resolveGitPath(cwd, requireString(payload, 'path'))
      const path = workspaceRoot === undefined ? resolvedPath : await ensureWorkspacePath(workspaceRoot, resolvedPath, true)
...
export function apply(ctx: Context, config?: SidebarConfig): void {
  ...
  // Real Cordis contexts expose provide(); the optional fallback keeps the
  // host half independently testable with the small structural contexts used
  // by route smoke tests.
  if (typeof (ctx as Context & { provide?: unknown }).provide === 'function') {
    ctx.provide('betterSidebarPresentation', presentationService)
  } else {
    ;(ctx as Context & { betterSidebarPresentation?: typeof presentationService }).betterSidebarPresentation = presentationService
  }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const base = sessionCwdOf(ctx, sessionId, url.searchParams.get('cwd') ?? undefined)
        const cwd = await resolveWorkspaceCwd(base, url.searchParams.get('workspaceRoot') ?? undefined)
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const safePath = await ensureWorkspacePath(cwd, path)
        if (!isWithin(cwd, safePath)) {
        ...
        if (url.searchParams.get('download') === '1') {
          headers['content-disposition'] = `attachment; filename*=UTF-8''${encodeURIComponent(basename(safePath))}`
        }
  ...
  const htmlHandler = async (req: SidebarHttpRequest, res: import('./context-types.ts').SidebarHttpResponse): Promise<void> => {
      if (!fence(req)) {
        res.writeHead(403)
        res.end('forbidden')
        return
  ...
  ctx.effect(() => {
    const legacy = ctx.webServer.register({ kind: 'prefix', path: '/sidebar/html', handler: htmlHandler })
    const workspace = ctx.webServer.register({ kind: 'prefix', path: '/sidebar/workspace-html', handler: htmlHandler })
  ...
  // Host-only user presentation feed. This is a deterministic service/WS seam;
  // it is not a model tool and never invokes Postman, REQ, ORCA or UI clicks.
  ...
  ctx.effect(() => ctx.webServer.registerUpgrade({
    ...
    handler: (req, socket, head) => {
      ...
      presentationWss.handleUpgrade(req as unknown as IncomingMessage, socket as unknown as Duplex, head as Buffer, (ws) => {
        void attachPresentation(presentationRegistry, ws, req)
      })
    },
