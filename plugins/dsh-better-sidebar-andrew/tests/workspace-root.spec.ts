...
import { listWorkspaceTargets, resolveWorkspaceCwd, sameWorkspacePath } from '../src/workspace-root.ts'

const pluginRoot = resolve(fileURLToPath(new URL('..', import.meta.url)))

describe('workspace/worktree selector authority', () => {
  it('uses Windows case-insensitive path rules only on Windows', () => {
    expect(sameWorkspacePath('C:/Users/Andrew/.dsh', 'c:/users/andrew/.dsh', 'win32')).toBe(true)
    expect(sameWorkspacePath('/Users/Andrew/.dsh', '/users/andrew/.dsh', 'linux')).toBe(false)
  })
  ...
  it.skipIf(process.platform !== 'win32')('accepts a Windows worktree path with different casing', async () => {
    const root = await realpath(await git.repoRoot(pluginRoot))
    const alternate = root.replace(/Andrew/i, 'andrew')
    await expect(resolveWorkspaceCwd(pluginRoot, alternate)).resolves.toBe(root)
  })
