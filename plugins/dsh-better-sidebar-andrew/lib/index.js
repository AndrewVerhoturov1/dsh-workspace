...
async function listDirectory(path, maxEntries = 1e3) {
	...
	try {
		...
		throw new SidebarError("fs-error", `cannot list "${path}": ${messageOf$1(error)}`, 400);
	}
	const rows = [];
	let overflow = 0;
	try {
...
function decodeHtmlUrl(pathname) {
	...
	if (workspace) {
		...
		if (workspaceRoot === void 0 || workspaceRoot === "") return {
			ok: false,
			status: 400,
...
async function ensureWorkspacePath(workspaceRoot, target, allowMissing = false) {
	const root = await realpath(requireAbsolute(workspaceRoot)).catch((error) => {
	...
	try {
		...
		if (!allowMissing) throw new SidebarError("forbidden", `path is unavailable: ${messageOf(error)}`, 403);
		const parent = await realpath(dirname(absolute)).catch((parentError) => {
	...
	if (!isWithin(root, resolved)) throw new SidebarError("forbidden", "path escapes the selected workspace", 403);
	return resolved;
}
...
/** Parse the porcelain worktree listing without invoking checkout. */
function parseWorktrees(output) {
	...
	} else if (current !== void 0 && line.startsWith("branch ")) {
...
/** List every physical worktree of the repository containing `cwd`. */
async function worktrees(cwd) {
	return parseWorktrees(await runGit(cwd, [
		"worktree",
		"list",
		"--porcelain"
	]));
...
*/
async function resolveWorkspaceCwd(baseCwd, requested) {
	if (requested === void 0 || requested === "") return baseCwd;
	const absolute = requireAbsolute(requested);
	const entries = await worktrees(await repoRoot(baseCwd).catch(() => baseCwd));
	const requestedReal = await realpath(absolute).catch(() => void 0);
...
async function listWorkspaceTargets(baseCwd) {
	const authority = await repoRoot(baseCwd).catch(() => baseCwd);
	const rootReal = await realpath(authority).catch(() => authority);
	const entries = await worktrees(authority);
...
var SidebarPresentationRegistry = class {
...
function titleOf(kind, target, supplied) {
	if (supplied !== void 0 && supplied.trim() !== "") return supplied.trim();
	if (kind === "url") try {
...
function createBetterSidebarPresentationService(options) {
	...
	return {
		async present(input) {
			...
			if (input.kind !== "html" && input.kind !== "url" && input.kind !== "file" && input.kind !== "folder") throw new Error("unsupported presentation kind");
			const workspaceRoot = await resolveWorkspaceCwd(await resolveSessionCwd(input.sessionId), input.workspaceRoot);
			...
			if (input.kind === "url") {
				...
				try {
					...
					throw new Error("presentation URL is invalid");
				}
				if (parsed.protocol !== "http:" && parsed.protocol !== "https:") throw new Error("presentation URL must use http or https");
				target = parsed.href;
			} else {
				target = await ensureWorkspacePath(workspaceRoot, requireAbsolute(target));
				const info = await stat(target);
				if (input.kind === "folder") {
					if (!info.isDirectory()) throw new Error("presentation target is not a directory");
				} else if (!info.isFile()) throw new Error("presentation target is not a file");
			}
			return {
				...registry.enqueue({
					action: "present",
					sessionId: input.sessionId,
					workspaceRoot: input.workspaceRoot === void 0 ? void 0 : workspaceRoot,
					kind: input.kind,
					target,
...
async function resolveGitPath(cwd, raw) {
	...
	const local = requireAbsolute(join(cwd, raw));
...
function buildApi(ctx, ptyManager, agentPtyRegistry, resolved, terminalShell, getSettings) {
	const cwdOf = async (payload) => {
		const sessionId = requireString(payload, "sessionId");
		const record = payload;
		const base = sessionCwdOf(ctx, sessionId, typeof record?.cwd === "string" && record.cwd !== "" ? record.cwd : void 0);
		const requested = typeof record?.workspaceRoot === "string" && record.workspaceRoot !== "" ? record.workspaceRoot : void 0;
		const cwd = await resolveWorkspaceCwd(base, requested);
		return {
			sessionId,
			cwd,
			...requested === void 0 ? {} : { workspaceRoot: cwd }
		};
	};
	...
	return {
		...
		"fs.read": async (payload) => {
			const { cwd, workspaceRoot } = await cwdOf(payload);
			const resolvedPath = await resolveGitPath(cwd, requireString(payload, "path"));
			const { content, truncated, binary, size, head } = await readText(workspaceRoot === void 0 ? resolvedPath : await ensureWorkspacePath(workspaceRoot, resolvedPath), resolved.readLimit);
			if (binary) return {
				kind: "binary",
				size,
				truncated,
		...
		"fs.write": async (payload) => {
			const { cwd } = await cwdOf(payload);
			const path = await ensureWorkspacePath(cwd, requireAbsolute(requireString(payload, "path")), true);
		...
		"git.diff": async (payload) => {
			const { cwd, workspaceRoot } = await cwdOf(payload);
			const record = payload;
			const resolvedPath = record.path === void 0 ? void 0 : await resolveGitPath(cwd, requireString(payload, "path"));
			return { diff: await diff(cwd, resolvedPath === void 0 || workspaceRoot === void 0 ? resolvedPath : await ensureWorkspacePath(workspaceRoot, resolvedPath, true), record.staged === true) };
		},
		...
		"git.worktrees": async (payload) => {
			const { cwd } = await cwdOf(payload);
			return listWorkspaceTargets(cwd);
		},
		...
		"git.discard": async (payload) => {
			const { cwd, workspaceRoot } = await cwdOf(payload);
			const resolvedPath = await resolveGitPath(cwd, requireString(payload, "path"));
			await discard(cwd, workspaceRoot === void 0 ? resolvedPath : await ensureWorkspacePath(workspaceRoot, resolvedPath, true));
			return { ok: true };
		},
		...
		"git.show": async (payload) => {
			const { cwd, workspaceRoot } = await cwdOf(payload);
			const resolvedPath = await resolveGitPath(cwd, requireString(payload, "path"));
			const path = workspaceRoot === void 0 ? resolvedPath : await ensureWorkspacePath(workspaceRoot, resolvedPath, true);
...
function apply(ctx, config) {
	...
	const presentationService = createBetterSidebarPresentationService({
		...
		resolveSessionCwd: async (sessionId) => sessionCwdOf(ctx, sessionId)
	});
	if (typeof ctx.provide === "function") ctx.provide("betterSidebarPresentation", presentationService);
	else ctx.betterSidebarPresentation = presentationService;
		...
		handler: async (req, res) => {
			...
			try {
				...
				const { path, size } = await writeWorkspaceUpload({
					cwd: await resolveWorkspaceCwd(sessionCwdOf(ctx, sessionId, url.searchParams.get("cwd") ?? void 0), url.searchParams.get("workspaceRoot") ?? void 0),
					dir,
					relativePath,
					chunks: req,
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				const cwd = await resolveWorkspaceCwd(sessionCwdOf(ctx, sessionId, url.searchParams.get("cwd") ?? void 0), url.searchParams.get("workspaceRoot") ?? void 0);
				const safePath = await ensureWorkspacePath(cwd, requireAbsolute(raw));
				if (!isWithin(cwd, safePath)) throw new SidebarError("fs-error", "media path outside the session working directory", 403);
				const info = await stat(safePath);
				if (!info.isFile() || info.size > resolved.mediaLimit) throw new SidebarError("fs-error", "not a file or too large", 400);
				const type = mediaTypeForPath(safePath);
				const body = await readFile(safePath);
				const headers = {
					"content-type": type,
					"cache-control": "no-cache"
				};
				if (url.searchParams.get("download") === "1") headers["content-disposition"] = `attachment; filename*=UTF-8''${encodeURIComponent(basename(safePath))}`;
	...
	const htmlHandler = async (req, res) => {
		...
		try {
			...
			const cwd = await resolveWorkspaceCwd(sessionCwdOf(ctx, sessionId), decoded.ref.workspaceRoot);
			const absolute = await ensureWorkspacePath(cwd, requireAbsolute(path));
	...
	ctx.effect(() => {
		const legacy = ctx.webServer.register({
			kind: "prefix",
			path: "/sidebar/html",
			handler: htmlHandler
		});
		const workspace = ctx.webServer.register({
			kind: "prefix",
			path: "/sidebar/workspace-html",
			handler: htmlHandler
		});
		return () => {
			legacy();
	...
	}, "dsh-better-sidebar: /sidebar/html preview routes");
	...
	const presentationWss = new WebSocketServer({ noServer: true });
	ctx.effect(() => ctx.webServer.registerUpgrade({
		path: "/sidebar/ws/presentations",
		handler: (req, socket, head) => {
			if (!fence(req)) {
				socket.destroy();
				return;
			}
			presentationWss.handleUpgrade(req, socket, head, (ws) => {
				attachPresentation(presentationRegistry, ws, req);
			});
		}
	}), "dsh-better-sidebar: presentation WebSocket");
	ctx.effect(() => () => {
		toolsDisposers?.();
		...
		wss.close();
		agentListWss.close();
		presentationWss.close();
		presentationRegistry.dispose();
...
/** Attach one sidebar session to the queued host-only presentation feed. */
async function attachPresentation(registry, ws, req) {
	try {
		const sessionId = new URL(req.url ?? "/", "http://dsh.internal").searchParams.get("sessionId");
