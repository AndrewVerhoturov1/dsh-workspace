...
После `TEXT_RESULT_DURABLE` Harness сохраняет exact `assistantText` в session-scoped reply slot, привязанный к текущему REQ. Перед final response Luna обязана передать candidate final text в `postman_ask_validate_reply(request_id, text)`. Validator использует прямое строковое равенство без trim/whitespace/Markdown normalization и без отдельного SHA-gate. Только `EXACT_REPLY_MATCH` разрешает final response, причём финальный content должен быть ровно тем же candidate без вступления, нумерации, заключения или переформатирования. `EXACT_REPLY_MISMATCH` требует повторно взять исходный `assistantText` из terminal result и перепроверить; unavailable/request mismatch/invalid означают `STOP`.
