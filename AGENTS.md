...
Исполнитель сохраняет только аварийные предохранители: base должен быть `main`; primary worktree `C:\Users\andre\.dsh` не удаляется, не очищается и не переключается автоматически; dirty secondary worktree оставляется с warning; local/remote head branch удаляется только если всё ещё указывает на exact PR head SHA. После завершения merge/fetch executor может безопасно fast-forward'нуть primary worktree только когда он уже находится на `main`, через `git merge --ff-only refs/remotes/origin/main`. Незакоммиченные изменения, не пересекающиеся с входящими изменениями `main`, сохраняются; если Git отказывается от fast-forward, primary остаётся нетронутым с warning и уже успешный GitHub merge не отменяется. Если primary не на `main`, executor не делает checkout/switch и возвращает warning. Отсутствующие worktree/ветки считаются нормальным уже очищенным состоянием. Cleanup и primary sync выполняются best-effort. Запрещены `reset --hard`, `stash`, `git clean` и force push.
