...
`resume_request.ps1`, PREPARE, TEST, PUBLISH и `integrate_result.ps1` сохраняются как legacy/manual explicit finalization для artifact durable result. Они не являются частью normal `@Postman` или `@PostmanAsk` flow. При manual finalization TestScript/TestSpec передаются argv-safe. В Leader flow Host `postman_task_prepare` до Bridge создаёт и публикует одну task branch/clean worktree от exact `origin/preview`; Bridge публикует REQ туда через Host, не в `main`. Worker использует тот же worktree на опубликованном REQ commit, не создаёт вторую ветку. Перед отдельным commit/push/PR реализации REQ transport-файлы очищаются; SHA-pinned URL прежних REQ и `--chat` сохраняются. Legacy CLI default `main` применим только вне Leader. Normal transport сам не применяет package и не создаёт implementation commit/PR. Настоящий transport failure (`ok=false`) остаётся strict fail-closed: STOP без fallback/повторного Send того же REQ.
