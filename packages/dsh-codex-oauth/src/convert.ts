...
async function prepareRequestImages(
  ...
  for (const [index, ref] of orderedRefs.entries()) versions.set(ref.attachmentId, prepared[index] as RequestImageAttachment)
  return versions
}
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'image': {
        ...
        if (version === undefined) throw new LlmError('Harness image was not prepared for the pi-ai request', 'UNSUPPORTED_CONTENT')
...
async function contextWithImages(options: GenerateOptions, attachments: AttachmentStore): Promise<PiContext> {
  assertSupportedImageRoles(options.messages)
  const requestMessages = offloadRequestImagesWithPolicy(options.messages, {
    representation: 'base64',
    maxBytes: MAX_REQUEST_IMAGE_BYTES,
    byteQuantum: 1,
    byteLength: ref => Math.min(ref.bytes, REQUEST_IMAGE_POLICY.maxBytes),
  })
  const requestImages = await prepareRequestImages(requestMessages, attachments, REQUEST_IMAGE_POLICY, options.signal)
...
export async function toCodexContext(options: GenerateOptions, attachments?: AttachmentStore): Promise<PiContext> {
  if (options.messages.some(message => contentHasImage(message.content))) {
    if (attachments === undefined) {
      throw new LlmError('pi-ai image input requires the durable attachment service', 'UNSUPPORTED_CONTENT')
    }
    return contextWithImages(options, attachments)
  }
  const toolNames = new Map<CallId, string>()
  const messages: PiMessage[] = []
