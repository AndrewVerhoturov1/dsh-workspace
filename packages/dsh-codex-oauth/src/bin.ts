 ...
 * interactive output reaches the user. The selected Harness home is explicit
 * in the child environment so approvals and installation target one profile.
 * @param profile - profile to initialize and modify.
 * @param home - Harness home that owns the profile.
 * @returns the child exit code.
 */
function runPluginAdd(profile: string, home: string): number {
...
async function install(options: Options): Promise<void> {
  ...
  if (options.dryRun) {
    ...
    console.log(`dry run: would run with DSH_HOME=${options.home}: dsh plugin --profile ${options.profile} add ${INSTALL_SPEC}`)
    return
  }
  const step = await ensureAllowBuilds(workspaceFile)
  console.log(`✔ ${step.text}`)
  const code = runPluginAdd(options.profile, options.home)
  if (code !== 0) {
    process.exit(code)
  }
