...
function reportOpenFailure(reporter: LoginReporter | undefined, error: unknown): void {
  ...
  reporter?.line(`dsh-codex-oauth: could not open the browser automatically: ${detail}`)
}
        ...
        case 'manual_code':
          await new Promise<void>((resolve, reject) => {
            if (prompt.signal === undefined) {
              reject(new Error('dsh-codex-oauth: browser login supplied no callback-completion signal'))
              return
            }
            const cleanup = (): void => {
            ...
            const onLoginAbort = (): void => {
              ...
              reject(reason instanceof Error ? reason : new Error('dsh-codex-oauth: login cancelled'))
            }
            if (signal?.aborted) {
              onLoginAbort()
              return
